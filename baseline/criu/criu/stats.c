...
void prst_display_mem_timing_stats (void) {
    ...
    tm = prst_get_timing (TIME_MEM_PREP);
    time = tm->total.tv_sec * USEC_PER_SEC + tm->total.tv_usec;
    pr_debug ("PCLive: mem prep time: %ld us\n", time);

}

void prst_display_others_timing_stats (void) {

    unsigned long time;
    struct timing *tm;

    tm = prst_get_timing (TIME_OTHERS_PREP);
    time = tm->total.tv_sec * USEC_PER_SEC + tm->total.tv_usec;
    pr_debug ("PCLive: mem prep time: %ld us\n", time);

}

void prst_reset_others_timing_stats (void) {
    prst_stats->timings[TIME_OTHERS_PREP].start.tv_sec = 0;
    prst_stats->timings[TIME_OTHERS_PREP].start.tv_usec = 0;
    prst_stats->timings[TIME_OTHERS_PREP].total.tv_sec = 0;
    prst_stats->timings[TIME_OTHERS_PREP].total.tv_usec = 0;
}

void prst_reset_mem_timing_stats (void) {
    int i;
    for (i = 0; i < RESTORE_TIME_NS_STATS; i++) {
        if (i == TIME_OTHERS_PREP)
            continue;
