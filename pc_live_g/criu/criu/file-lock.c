...
struct list_head *file_lock_list = NULL;

int file_lock_shalloc (bool is_restore) {

    if (is_restore)
        file_lock_list = fshmalloc (sizeof (*file_lock_list));
    else
        file_lock_list = xmalloc (sizeof (*file_lock_list));

    if (file_lock_list == NULL) {
        pr_perror ("Can't allocate from fixed shared/ private space\n");
        return 1;
    }
...
// Function to check whether file lock entry is already present from the
// previous iteration.
static int search_and_update_one_file_lock (ProtobufCMessage *base) {

    FileLockEntry *fle;
    struct file_lock_rst *fl;

    // Decode the image.
    fle = pb_msg (base, FileLockEntry);

    // Search for this id.
    if (!list_empty(file_lock_list)) {
        list_for_each_entry (fl, file_lock_list, l) {
            if (fl->fle->id == fle->id) {
                memcpy (fl->fle, fle, sizeof (*fle));
                return 1;
            }
        }
    }
...
struct collect_image_info file_locks_cinfo = {
    ...
    .search_and_update = search_and_update_one_file_lock,
