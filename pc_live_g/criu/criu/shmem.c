...
int collect_shmem(int pid, struct vma_area *vma)
{
	...
	si = fshmalloc(sizeof(struct shmem_info));
	if (!si)
		return -1;
...
static int do_restore_shmem_content(struct vma_area *vma, void *addr, unsigned long size, unsigned long shmid)
{
	int ret = 0;
	struct page_read pr;
	...
	while (1) {
		...
		pr.read_pages(&pr, vma, vaddr, nr_pages, addr + vaddr, 0);
...
int restore_shmem_content(struct vma_area *vma, void *addr, struct shmem_info *si)
{
	return do_restore_shmem_content(vma, addr, si->size, si->shmid);
}

int restore_sysv_shmem_content(struct vma_area *vma, void *addr, unsigned long size, unsigned long shmid)
{
	return do_restore_shmem_content(vma, addr, round_up(size, PAGE_SIZE), shmid);
}

int restore_memfd_shmem_content(struct vma_area *vma, int fd, unsigned long shmid, unsigned long size)
{
	void *addr = NULL;
	int ret = 1;
	 ...
	 */
	if (do_restore_shmem_content(vma, addr, round_up(size, PAGE_SIZE), shmid) < 0) {
...
static int open_shmem(int pid, struct vma_area *vma)
{
	...
	if (restore_shmem_content(vma, addr, si) < 0) {
		pr_err("Can't restore shmem content\n");
		goto err;
	}
