...
static int dump_one_nf(struct nlmsghdr *hdr, struct ns_id *ns, void *arg)
{
    ...
    if (opts.use_xfer_server) {
        ret = write_img_buf_xfer(img, hdr, hdr->nlmsg_len);
    } else {
        ret = write_img_buf(img, hdr, hdr->nlmsg_len);
    }
	if (ret < 0)
		return -1;
...
static int read_links(struct ns_id *ns)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &nde, PB_NETDEV, false /*XXX: Later Check*/);
		if (ret <= 0)
			break;
...
int read_net_ns_img (void) {
	...
	for (ns = ns_ids; ns != NULL; ns = ns->next) {
        ...
        // Delete any netns data processed in previous migration iteration.
        if (ns->net.netns)
...
static int iptables_restore(bool ipv6, char *buf, int size)
{
     ...
     * XXX: Shiv
     *
     * Removed -w option because it was giving error.
     *
     */
 ...
 * TODO: Shiv
 * 
 * "iptables-restore" command is not working, so removed that and executed
 * commands one by one (instead of in one go) using "iptables" command.
 */
static int iptables_network_unlock_internal (void) {
     ...
     * XXX: Shiv
     *
     * Below commented code is not working correctly, i.e. it is not removing
     * added rules and chain.
     *
    char conf[] = "*filter\n"
              ":CRIU - [0:0]\n"
              "-D INPUT -j CRIU\n"
    ...
    */
    ...
    /**
     * Delete rules and chain using "iptables(6)" as it is done in function
     * "iptables_connection_switch_raw"
     *
     * You have do the same given in "conf" variable above.
     *
     * iptable rules to be executed are:
     *      1.) First delete sub-chain CRIU from INPUT chain
     *          iptables -t filter -D INPUT -j CRIU
     *      2.) Then delete sub-chain CRIU from OUTPUT chain
     *          iptables -t filter -D OUTPUT -j CRIU
     *      3.) Then flush the CRIU chain (will delete all rules on this chain)
     *          iptables -t filter -F CRIU
     *      4.) And finally, delete the CRIU chain
     *          iptables -t filter -X CRIU
     *
     */
    char *rules[4] = {
        "iptables -t filter -D INPUT -j CRIU",
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (ret < 0 || !WIFEXITED(ret) || WEXITSTATUS(ret)) {
            pr_err("Unable to delete rule number %d using iptables\n", (i + 1));
            return -1;
        }
