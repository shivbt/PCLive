...
static CgControllerEntry **controllers = NULL;
static char *cg_yard;
...
// Added by Shiv
unsigned long dump_cgroup_args_size (void) {

    return sizeof (struct parasite_dump_cgroup_args);
...
int dump_task_cgroup (struct pstree_item *item, u32 *cg_id
        , struct parasite_dump_cgroup_args *args) {
    ...
    // Shiv:
    // To dump cgroup with --dump-info option even when core dumping is not
    // mentioned (see *dump_cgroup* lable in *pre_dump_one_task_general* function)
    // because both have some implementational dependency.
    if (cg_id != NULL)
        *cg_id = cs->id;
...
static int prepare_cgroup_sfd (CgroupEntry *ce) {
    ...
    i = get_service_fd (CGROUP_YARD);
    if (i > 0) {
        goto skip_cg_yard_open;
    }
...
int prepare_cgroup (void) {
	...
	ret = pb_read_one_eof(img, &ce, PB_CGROUP, false /*XXX: Later Check*/);
	close_image(img);
    ...
    // Check whether cgroup is processed in previous iteration. If yes, then
    // free previous allocated data.
    if (controllers) {
        for (i = 0; i < n_sets; i++) {
            xfree (rst_sets[i]->ctls);
        }
        xfree (rst_sets);
        xfree (controllers);
        controllers = NULL;
