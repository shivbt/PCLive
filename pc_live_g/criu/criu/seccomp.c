 ...
 */
static void try_use_tsync (struct seccomp_entry *leader
    ...
    list_for_each_entry(thread_item, &(item->threads), thread_list) {
		...
		entry = seccomp_find_entry(thread_item->thread.real);
		BUG_ON(!entry);
...
int seccomp_read_image(void)
{
	...
	ret = pb_read_one_eof(img, &seccomp_img_entry, PB_SECCOMP, false /*XXX: Called in init iter only*/);
	close_image(img);
	if (ret <= 0)
...
int seccomp_prepare_threads (struct pstree_item *item
	...
	for (i = 0; i < item->nr_threads; i++) {
		...
		if (thread_core->seccomp_filter >= seccomp_img_entry->n_seccomp_filters) {
			pr_err("Corrupted filter index on thread of %d (%u > %zu)\n"
                    , vpid (item), thread_core->seccomp_filter
		...
		if (sf->filter.len % (sizeof(struct sock_filter))) {
			pr_err("Corrupted filter len on thread of %d (index %u)\n"
                    , vpid (item), thread_core->seccomp_filter);
			return -1;
		}
		...
		if (!args->seccomp_filters) {
			pr_err("Can't allocate %zu bytes for filters on thread of %d\n"
                    , rst_size, vpid (item));
			return -ENOMEM;
		}
