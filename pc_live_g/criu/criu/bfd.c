...
static int mmap_pages_file_content (struct xbuf *xb, int fd) {
...
int mmap_pages_file (struct bfd *f) {

    if (mmap_pages_file_content (&f->b, f->fd)) {
        close_safe (&f->fd);
        return -1;
    }
