...
static int dump_ipc_sem_set(struct cr_img *img, const IpcSemEntry *sem)
{
    ...
    if (opts.use_xfer_server) {
        ret = write_img_buf_xfer(img, values, rounded);
    } else {
...
static int dump_ipc_msg_queue_messages(struct cr_img *img, const IpcMsgEntry *msq, unsigned int msg_nr)
{
	...
	for (msg_cnt = 0; msg_cnt < msg_nr; msg_cnt++) {
        ...
        if (opts.use_xfer_server) {
            ret = write_img_buf_xfer(img, message->mtext, rounded);
        } else {
...
static int prepare_ipc_sem(int pid)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &sem, PB_IPC_SEM, false /*XXX: Later Check*/);
		if (ret < 0) {
			ret = -EIO;
			goto err;
		}
...
static int prepare_ipc_msg(int pid)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &msq, PB_IPCNS_MSG_ENT, false /*XXX: Later Check*/);
		if (ret < 0) {
...
static int prepare_ipc_shm_pages(struct cr_img *img, const IpcShmEntry *shm)
{
	...
	if (shm->has_in_pagemaps && shm->in_pagemaps)
		ret = restore_sysv_shmem_content(NULL, data, shm->size, shm->desc->id);
	else
...
static int prepare_ipc_shm(int pid)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &shm, PB_IPC_SHM, false /*XXX: Later Check*/);
		if (ret < 0) {
