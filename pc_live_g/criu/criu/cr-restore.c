...
static inline int stage_current_participants(int next_stage) {
    ...
    case CR_STATE_PRE_RESTORE:
        /**
         * XXX: Shiv
         * Same as below case , i.e. CR_STATE_RESTORE
         */
...
static int crtools_prepare_shared (bool init_iter, bool last_iter) {

	if (prepare_memfd_inodes(init_iter))
		return -1;

	if (prepare_files (init_iter))
		return -1;

	/* We might want to remove ghost files on failed restore */
	if (collect_remaps_and_regfiles(init_iter))
		return -1;
    ...
    if (last_iter && collect_binfmt_misc())
		return -1;

	if (last_iter && prepare_apparmor_namespaces())
		return -1;

	if (init_iter && tty_prep_fds())
		return -1;
...
static int root_prepare_shared (bool init_iter, bool last_iter) {
	...
	if (prepare_remaps (init_iter))
		return -1;

    if (seccomp_read_image())
        return -1;

    // Process cinfos in last iteration only.
    if (last_iter && collect_images(cinfos, ARRAY_SIZE(cinfos)))
		return -1;
	...
	for_each_pstree_item(pi) {
        ...
        // Skip deleted tasks for this iteration.
    ...
    // Prepare restore blob only in first iteration.
    if (init_iter) {
    ...
    /**
     * TODO: Shiv
     * Currently processing the info in init iteration... will remove this after
     * testing and will only process these info in final iteration.
     */
    if (!init_iter)
        goto skip_other_info;
...
static int restore_one_sigaction(int sig, struct cr_img *img, int pid)
{
	...
	ret = pb_read_one_eof(img, &e, PB_SIGACT, false /*XXX: Later Check*/);
	if (ret == 0) {
...
static int open_cores (int pid, CoreEntry *leader_core) {
    ...
    i = 0;
    list_for_each_entry(thread_item, &(current->threads), thread_list) {

		tpid = thread_item->thread.ns[0].virt;
...
struct task_entries *task_entries = NULL;
...
/**
 * XXX: Shiv
 * Commented this to suppress gcc warning.
...
static inline int fork_with_pid(struct pstree_item *item)
{
	...
	if (item->pid->state != TASK_HELPER) {
        ...
        */
        ca.core = NULL;     // XXX: Shiv: Added to remove core related thing in init iteration.
	} else {
		/*
...
static int sigchld_process(int status, pid_t pid)
{
	...
	if (WIFEXITED(status)) {
        // XXX: Shiv
        // The tasks can be disappeared in successive iteration with graceful
        // exit(0) hence in that case it will not be error.
...
/**
 * Function to check if a task (virtual pid) is supposed to be exited or killed.
 * This can happen in iterative parallel restore during task tree change.
 */
static inline bool task_supposed_to_be_killed (pid_t vpid) {

    struct pid *found_pid = pstree_pid_by_virt (vpid);
    bool found = found_pid ? (found_pid->is_deleted) : false;
    return found;
...
static int prst_open_core (struct pstree_item *pi, struct cr_clone_arg *ca) {
    ...
    if (open_core (pid, &ca->core))
        return 1;
    if (check_core (ca->core, pi))
        return 1;

    pi->pid->state = ca->core->tc->task_state;
    rsti(pi)->cg_set = ca->core->tc->cg_set;

    if (pi->pid->state != TASK_DEAD && !task_alive(pi)) {
        pr_err("Unknown task state %d\n", pi->pid->state);
        return -1;
    }
...
static inline void __prst_update_child (struct pstree_item *child) {

    if (!child->task_created || child->pid->is_deleted)
        return;

    if (child->task_updated)
        return;

    pr_debug ("Task %d will be signaled for update\n", vpid(child));
    prst_send_command_async (child, PRST_UPDATE);
    child->task_updated = true;
...
static int create_children_and_session(bool new_task) {
	...
	list_for_each_entry(child, &current->children, sibling) {
        ...
        if (child->task_created) {
            __prst_update_child (child);
            continue;
        }
...
// This function is copy of restore_task_with_children fn with only global
// info processing in successive iteration.
//
// XXX:
// 1.) Change this function to allow global info processing only needed in
//     successive iterations.
// 2.) In case of successive iteration, in *create_children_* fn -> go for child
//     creation if it is not created otherwise signal the child task to update
//     info for this iteration.
// 3.) ...
int prst_update_task_with_children (struct cr_clone_arg *ca) {
    ...
    if (current->parent == NULL) {
        if (root_prepare_shared (false, current->is_last_iter))
            goto err;
    }
    ...
    if (prst_update_mappings (current))
        goto err;
    prst_display_mem_timing_stats (root_item->glob_iter);
    prst_reset_mem_timing_stats ();

    // TODO: Later -> Here you can add some time info as well just like TIME_FORK.
    timing_start(TIME_FORK);

    if (create_children_and_session(false))
        goto err;
    ...
    /**
     * TODO: Later
     * Check whether you need to unmap gaurd pages in successive iteration for
     * alreday created tasks. I think no, But first check with exp ???
     */
    ...
    if (current->parent == NULL) {
        ...
        int stage = CR_STATE_PRE_RESTORE;
        /*
        ...
        if (current->is_last_iter) {
            stage = CR_STATE_RESTORE;
            fini_restore_mntns();
        }

        __restore_switch_stage (stage);
...
static int restore_task_with_children(void *_arg)
{
    ...
    // @Strong Assumption:
    //
    // This function will called initially for each process then will be called
    // for newly created process for successive iterations.
    //
	/* Restore root task */
	if (current->parent == NULL) {
		if (join_namespaces()) {
		...
		if (root_prepare_shared(true, false))
			goto err;
	...
	/**
     * TODO: Shiv
     * Removing this prepare_sigactions from here for now...
     *
	...
	if (create_children_and_session(true))
		goto err;
    ...
    // XXX: Shiv
    // Make the variable "restoration_point = 1" to avoid opening parent image
    // directories in case of pipelined restore (see read_pagemap_page fn).
    restoration_point = 1;
    pr_debug ("Task %d may wait for next message\n", vpid(current));
    if (!(current->is_last_iter)) {

        // Free core entry openend from init iteration (with fork_with_pid).
        if (ca->core)
            core_entry__free_unpacked (ca->core, NULL);

        // Wait for each iteration updates and then return here after final
        // iteration update.
        if (prst_handle_commands (current))
            goto err;

        pr_debug ("All iterations are over for the task %d\n", vpid(current));

    } else {
        // By default set the ack as 1 for all tasks.
        futex_set_and_wake (&(current->prst_msg.ack), 1);
    }

    // Finally, open the core image for last iteration.
    if (prst_open_core (current, ca)) {
        goto err;
    }

	// TODO: Shiv
	// Moved the above prepare_sigactions to here to catch seg fault bug...
	if (prepare_sigactions(ca->core) < 0)
		goto err;

	// Close named fifo pipes.
    sfds_protected = false;
	close_fifo_pipe_file (FIFO_PIPE_FD_OFF);
	close_fifo_pipe_file (DATA_FIFO_PIPE_FD_OFF);
    sfds_protected = true;

    // You will not come here in first iteration, if iterations > 1.
    //
    // TODO: Next Later
    //
    // Here currently I am assuming that no new task is created in any iteration
    // to be sure that same function will be called, i.e. prst_update_task_with_children
    // for all other iteration except first. Need to handle the case when
    // some new tasks are created and it is executing restore_one_task from here
    // and previously created tasks (in prev iteration) is executing restore_one_task
    // from prst_update_task_with_children.
    //
    // WILL THAT MAKE DIFFERENCE ??? CHECK ???
    //
...
static int attach_to_tasks (bool root_seized) {
	...
	for_each_pstree_item(item) {
        ...
        // XXX: Shiv 1
        //
        // Temporary workaround to get real tid of threads...
        threads = xmalloc (item->nr_threads * sizeof(struct pid));
        if (!threads)
            return -1;
		...
		if (item->nr_threads == 1) {
            ...
            thread_item->thread.real = item->pid->real;
            threads[0].real = item->pid->real;

        } else {

            //if (parse_threads(item->pid->real, &item->threads, &item->nr_threads)) {
            if (parse_threads(item->pid->real, &threads, &item->nr_threads)) {
                ret = -1;
                goto error;
            }
            ...
            // XXX: Shiv 2
            //
            // Temporary workaround to get real tid of threads into already
            // processed thread list data structure (before this it just have
            // virtual tid, i.e. w.r.t. namespace not global).
            th = 0;
            list_for_each_entry(thread_item, &(item->threads), thread_list) {
                thread_item->thread.real = threads[th++].real;
            }
...
static int clear_breakpoints (void) {
	...
	for_each_pstree_item(item) {
		...
		//for (i = 0; i < item->nr_threads; i++)
        list_for_each_entry(thread_item, &(item->threads), thread_list) {
			ret |= ptrace_flush_breakpoints(thread_item->thread.real);
        }
...
static int finalize_restore_detach (void) {
	...
	for_each_pstree_item(item) {
        ...
        list_for_each_entry(thread_item, &(item->threads), thread_list) {
			...
			pid = thread_item->thread.real;
			if (pid < 0) {
				pr_err("pstree item has unvalid pid %d\n", pid);
				continue;
			}

			if (arch_set_thread_regs_nosigrt(&(thread_item->thread))) {
...
// TODO: Later
// Copy of "restore_root_task" before the "skip_ns_bouncing" label.
static int pre_restore_root_task (struct pstree_item *init, bool init_iter) {
...
others:
	...
	if (prepare_namespace_before_tasks(init_iter))
		return -1;
...
// TODO: Later
// This is the copy below the "skip_ns_bouncing" label from "restore_root_task"
static int final_restore_root_task (void) {
	...
	gettimeofday(&t, NULL);
	pr_info("Restore finished successfully. Tasks resumed (timestamp: %ld.%ld).\n", t.tv_sec, t.tv_usec);
	write_stats(RESTORE_STATS);
...
static int restore_root_task (struct pstree_item *init) {
	...
	if (prepare_namespace_before_tasks(true))
		return -1;
...
int cr_restore_tasks(void)
{
    ...
    // XXX: Shiv
    //
    // Allocate 2GB shared memory space that will be used for addition, deletion
    // and updation of shared data structures in case of iterative migration.
    //
    // Note: This is needed because all shared data structure allocation is
    // calling fshmalloc rather than shmalloc.
    if (fshmem_init ()) {
        goto err;
    }

    // XXX: Shiv
    // This must be called below fshmem_init fn because it allocate from the
    // fixed shared memory space.
    if (fdesc_shalloc ())
        goto err;
    ...
    if (file_lock_shalloc (true))
        goto err;

    if (fifo_head_shalloc ())
        goto err;

    if (pd_hash_fifo_shalloc ())
        goto err;

    if (pipes_shalloc ())
        goto err;

    if (pd_hash_pipes_shalloc ())
        goto err;

    if (inet_ports_shalloc ())
        goto err;

    if (packets_shalloc ())
        goto err;

    if (tcp_repair_sk_shalloc ())
        goto err;

    if (unix_ghosts_shalloc ())
        goto err;

    if (unix_sockets_shalloc (true))
        goto err;

    if (collected_ttys_shalloc ())
        goto err;

#ifdef CONFIG_BINFMT_MISC_VIRTUALIZED
    if (binfmt_misc_shalloc ())
        goto err;
#endif

#ifdef CONFIG_HAS_LIBBPF
    if (bpfmap_data_ht_shalloc ())
        goto err;
#endif

    if (init_pid_root_rb (true))
        goto err;

    if (check_img_inventory(/* restore = */ true, true) < 0)
		goto err;
	...
	if (crtools_prepare_shared(true, true) < 0)
		goto err;
 ...
 * TODO: Shiv
 *
 * 1.) Remove those functions which MUST be delayed to the final restore function.
 * 2.) Update each sub-functions or make new sub-functions (of cr_restore_tasks)
 *     to support our idea.
 * 3.) Add unix sockets/ pipes to talk among xfer-server, pre-restore process
 *     and its children.
 * 4.) Set "IMG_PR_RST_FD_OFF" value to point to the fd that will point to dir
 *     of current .img files EITHER in this function OR the calling function. [VERY IMPORTANT]
 * 5.) Test
 *
 * @Idea:
 *
 * 1.) There will be two stages in parallel-restore:
 *     1.) Pre-Restore stage:
 *         In this stage Initialization, allocation and update of associated
 *         data structures will happen by processing .img files.
 *     2.) Restore stage:
 *         In this stage along with above, processes will be made alive.
 *
 * 2.) Some sub-functions in this function must be called only once at first
 *     iteration like init_service_fd().
 *
 * XXX:
 * 1.) This function will be called from xfer-server code in each iteration.
 * 2.) This function is copy from cr_restore_tasks() with some modifications.
 * 3.) Work-dir-path will be same as xfer-server command work-dir-path.
 *
 */
int cr_pre_restore_tasks (int curr_iter, bool init_iter, bool last_iter) {
    ...
    restoration_point = opts.rp;
    is_xfer_process = false;
    //prestore_started = 1;
    ...
    /**
     * TODO: Later -> Currently Skipped... will check later
     *
     * Shiv: Check whether we need to initilize stats for pre-restore (intermediate)
     * OR Can it be moved for first time pre-restore call, i.e. when init is true.
     *
     */
    if (init_stats(RESTORE_STATS))
        goto err;
    ...
    // TODO: Shiv
    // Will check where to start the timing for restore ???
...
other_fn:
    ...
    // First reset some global info.
    root_ns_mask = 0;

    // Inventory.img file will always be transferred because it will be always
    // dumped at the sender side (It has very important feature which is UPTIME).
    // So, no need to check whether it will be sent or not.
	if (check_img_inventory(/* restore = */ true, init_iter) < 0) {
		goto err;
    }

	if (opts.cpu_cap & CPU_CAP_IMAGE) {
        if (cr_dinfo_bitmap [DUMP_CPUINFO]) {
		    if (cpu_validate_cpuinfo())
			    goto err;
        }
    }
	...
	if (prepare_pstree(init_iter) < 0) {
		goto err;
    }

    // XXX: Shiv
    // Skipping apparmor processing for parallel restore.
    if (crtools_prepare_shared(init_iter, last_iter) < 0)
        goto err;

    // TODO: Next -> Also update this in final iteration.
    // Prepare cgroup only in first iteration.
	if ((init_iter) && prepare_cgroup())
		goto clean_cgroup;

	if (last_iter && tty_init_restore())
		goto err;

    /**
     * XXX: Shiv
     * Omitting lazy page migration for now.
     *
	if (prepare_lazy_pages_socket() < 0)
		goto clean_cgroup;
     *
    ...
    if (init_iter) {
        ...
        ret = pre_restore_root_task (root_item, init_iter);
        ...
        /** XXX: Shiv
         *
         * If first iteration is also the last iteration, i.e. we are restoring
         * in just one iteration then don't wait for CR_STATE_PRE_RESTORE stage.
         */
        if (!last_iter) {
            futex_wait_until (&task_entries->start, CR_STATE_PRE_RESTORE);
            clean_pstree (last_iter);
        }

    } else {
        // First reset the stage to CR_STATE_FORKING.
        __restore_switch_stage_nw (CR_STATE_FORKING);

        // Now apply the updates for this iteration.
        ret = prst_apply_updates (curr_iter, last_iter);
    }

    // XXX: Shiv
    // Print used bytes so far on of the fixed shared memory region.
    pr_debug ("Used bytes of the fixed shared memory region: %ld (bytes)\n", fsh_get_used_bytes());

    if (!last_iter)
        return ret;

    ret = final_restore_root_task ();
...
static int prepare_posix_timers_from_fd(int pid, struct task_restore_args *ta)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &pte, PB_POSIX_TIMER, false /*XXX: Later Check*/);
		if (ret <= 0)
			break;
...
static int prepare_rlimits_from_fd(int pid, struct task_restore_args *ta)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &re, PB_RLIMIT, false /*XXX: Later Check*/);
		if (ret <= 0)
			break;
...
static int open_signal_image(int type, pid_t pid, unsigned int *nr)
{
	...
	*nr = 0;
	while (1) {
		...
		ret = pb_read_one_eof(img, &sie, PB_SIGINFO, false /*XXX: Later Check*/);
		if (ret <= 0)
			break;
...
static int prepare_signals (int pid, struct task_restore_args *ta
        , CoreEntry *leader_core) {
    ...
    list_for_each_entry(thread_item, &(current->threads), thread_list) {
		...
		if (!current->core[i]->thread_core->signals_p) /*backward compatibility*/
			ret = open_signal_image(CR_FD_PSIGNAL, thread_item->thread.ns[0].virt, &siginfo_priv_nr[i]);
		else
...
static int sigreturn_restore (pid_t pid, struct task_restore_args *task_args
        , unsigned long alen, CoreEntry *core) {
...
#endif
		thread_args[i].pid = thread_item->thread.ns[0].virt;
    ...
    // XXX: Shiv
    // Close the root directory of all dumped data.
