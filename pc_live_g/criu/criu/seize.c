...
static void unseize_task_and_threads (const struct pstree_item *item, int st) {
    ...
    list_for_each_entry(thread_item, &(item->threads), thread_list) {
        ...
        i++;
        if (i == 0) {
            continue;   // Skip the leader thread detachment here.
        }
        if (ptrace(PTRACE_DETACH, thread_item->thread.real, NULL, NULL)) {
            pr_perror("Unable to detach from %d", thread_item->thread.real);
        }
...
static inline bool thread_collected (struct pstree_item *i, pid_t tid) {
    ...
    list_for_each_entry(thread_item, &(i->threads), thread_list) {
        if (tid == thread_item->thread.real) {
            return true;
        }
    }
...
static int collect_threads (struct pstree_item *item) {
    ...
    */
    ...
    if (item->nr_threads == 0) {
        thread_item = create_thread_item (item, vpid (item), false);
	...
	for (i = 0; i < nr_threads; i++) {
        ...
        thread_item = create_thread_item (item, t_creds.s.vpid, false);
        thread_item->thread.real = pid;
        thread_item->thread.item = NULL;
        item->nr_threads++;
...
int collect_pstree(void)
{
    ...
    gettimeofday(&t, NULL);
    pr_info("Tasks are frozen (timestamp: %ld.%ld)\n", t.tv_sec, t.tv_usec);
