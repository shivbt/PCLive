...
static struct list_head *packets_list = NULL;

int packets_shalloc (void) {

    packets_list = fshmalloc (sizeof (*packets_list));
    if (packets_list == NULL) {
        pr_perror ("Can't allocate from fixed shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (packets_list);
    return 0;

}

// Function to check whether a skpacket entry is already present from the
// previous iteration.
static int search_and_update_one_packet (ProtobufCMessage *base) {

    SkPacketEntry *entry;
    struct sk_packet *pkt;

    // Decode the image.
    entry = pb_msg (base, SkPacketEntry);

    // Search for this id.
    if (!list_empty(packets_list)) {
        list_for_each_entry (pkt, packets_list, list) {
            if (pkt->entry->id_for == entry->id_for) {
                pkt->entry = entry;
                return 1;
            }
        }
...
static int collect_one_packet(void *obj, ProtobufCMessage *msg, struct cr_img *img) {
	...
	pkt->scm = NULL;
	pkt->data = fshmalloc (pkt->entry->length);
	if (pkt->data == NULL)
		return -1;
...
struct collect_image_info sk_queues_cinfo = {
    ...
    .search_and_update = search_and_update_one_packet,
...
int dump_sk_queue(int sock_fd, int sock_id)
{
	...
	while (1) {
        ...
        if (opts.use_xfer_server) {
            ret = write_img_buf_xfer(img_from_set(glob_imgset, CR_FD_SK_QUEUES), data, pe.length);
        } else {
