...
// Function to check whether a packet socket entry is already present from
// the previous iteration.
static int search_and_update_one_packet_sk (ProtobufCMessage *base) {

    struct packet_sock_info *si;
    PacketSockEntry *pse;
    struct file_desc *d;
    unsigned int id;
    int type;
    ...
    pse = pb_msg (base, PacketSockEntry);
    id = pse->id;
    type = (&packet_sock_desc_ops)->type;

    // Search for this id and type.
    d = find_file_desc_raw (type, id);
    if (d != NULL) {
        si = container_of (d, struct packet_sock_info, d);
        si->pse = pse;
        return 1;
    }
...
struct collect_image_info packet_sk_cinfo = {
    ...
    .search_and_update = search_and_update_one_packet_sk,
