...
struct page_read {
	...
	int (*read_pages)(struct page_read *, struct vma_area *vma, unsigned long vaddr, int nr, void *, unsigned flags);
    ...
    bool is_pagemaps_cached;    /* Shiv: True, if pagemaps are stored in array.
                                   Added for iterative restore with memory
                                   optimization */

    int last_pipe_data;		/* Shiv: To be used to terminate pagemap reading
				   while we are doing parallel restore with
				   memory optimization */
