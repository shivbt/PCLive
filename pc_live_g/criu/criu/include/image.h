...
#define VMA_PREV_PREMMAPED  (1 << 27)       // Shiv: Added to support pRestore
...
#define O_MIGRATE       (O_RDWR | O_CREAT | O_TRUNC)
...
extern int open_image_root_dir (char *dir);
extern int open_image_dir_at (int rfd, char *rel_path, bool check_parent);
extern void close_image_root_dir (void);

extern int open_fifo_pipe_file (char *dir, int fd_off);
extern void close_fifo_pipe_file (int fd_off);

extern struct cr_img ***dump_imgs;
extern void close_all_images (void);

extern int setup_images_dir (int iteration, bool is_restore, bool check_parent);

extern unsigned long page_ids;
...
extern struct cr_img *open_image_at(int dfd, char *img_name, int type, unsigned long flags, ...);
#define open_image(typ, flags, ...) open_image_at(-1, NULL, typ, flags, ##__VA_ARGS__)
extern int open_image_lazy(struct cr_img *img);
...
extern int write_img_buf_xfer(struct cr_img *, const void *ptr, int size);
