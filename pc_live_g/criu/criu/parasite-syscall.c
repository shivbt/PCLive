...
int parasite_dump_thread_leader_seized (struct parasite_ctl *ctl, int pid
        , CoreEntry *core) {
	...
	args = compel_parasite_args_s (ctl, PARASITE_DUMP_THREAD_ARGS_REGION);

    // Added by Shiv
    ctl->args_offset = ((unsigned long) args) - ((unsigned long) ctl->args);
	...
	ret = dump_thread_core(pid, core, args);
...
int parasite_dump_thread_seized (struct parasite_thread_ctl *tctl
        , struct parasite_ctl *ctl, int id, struct pid *tid, CoreEntry *core) {
    ...
    // Added by Shiv: Get the exact memory location for this thread arg
    ctl->args_offset = ((unsigned long) args) - ((unsigned long) ctl->args);
	...
	//tid->ns[0].virt = args->tid;  // XXX: Shiv -> Skipping this to avoid dump issue with core
    ret = dump_thread_core(pid, core, args);
    ctl->args_offset = 0;           // Added by Shiv
    return ret;
...
int parasite_dump_sigacts_seized (struct parasite_ctl *ctl
    ...
    args = compel_parasite_args_s (ctl, PARASITE_DUMP_SIGACTS_ARGS_REGION);
...
int parasite_dump_itimers_seized (struct parasite_ctl *ctl
        , struct pstree_item *item) {
    ...
    args = compel_parasite_args_s (ctl, PARASITE_DUMP_ITIMERS_ARGS_REGION);
...
static int encode_notify_thread_id (pid_t rtid, struct pstree_item *item
        , PosixTimerEntry *pte) {
    ...
    list_for_each_entry(thread_item, &(item->threads), thread_list) {
		...
		if (thread_item->thread.real != rtid)
			continue;

		vtid = thread_item->thread.ns[0].virt;
		break;
...
int parasite_dump_posix_timers_seized (struct proc_posix_timers_stat *proc_args
	...
	args = compel_parasite_args_s (ctl, PARASITE_POSIX_TIMERS_ARGS_REGION);
...
int parasite_dump_misc_seized (struct parasite_ctl *ctl
    ...
    ma = compel_parasite_args_s (ctl, PARASITE_DUMP_MISC_ARGS_REGION);

    // Added by Shiv
    ctl->args_offset = ((unsigned long) ma) - ((unsigned long) ctl->args);
...
struct parasite_tty_args *parasite_dump_tty (struct parasite_ctl *ctl, int fd
    ...
    p = compel_parasite_args_s (ctl, PARASITE_DUMP_TTY_ARGS_REGION);
	p->fd = fd;
	p->type = type;

    // Added by Shiv
    ctl->args_offset = ((unsigned long) p) - ((unsigned long) ctl->args);
...
int parasite_drain_fds_seized (struct parasite_ctl *ctl
        , struct parasite_drain_fd *dfds, int nr_fds, int off, int *lfds
        , struct fd_opts *opts) {

	int ret = -1, sk;
	struct parasite_drain_fd *args;

	args = compel_parasite_args_s (ctl, PARASITE_DRAIN_FDS_ARGS_REGION);
	args->nr_fds = nr_fds;
...
/* Shiv: This was officially the 50000'th line in the CRIU source code */

int parasite_dump_cgroup (struct parasite_ctl *ctl
        , struct parasite_dump_cgroup_args *cgroup) {
    ...
    // Added by Shiv
    // START
	ca = compel_parasite_args_s (ctl, PARASITE_DUMP_CGROUP_ARGS_REGION);
    // END

    ctl->args_offset = ((unsigned long) ca) - ((unsigned long) ctl->args);
 ...
 * XXX: Shiv
 *
 * Modified to store all data in non-overlapping manner. This is required for
 * cases where you want to dump pages info after dumping the all global info
 * either in pre-dump or in dump.
 *
 */
void parasite_ensure_args_size (unsigned long sz, bool dump_pages_later) {
...
static int parasite_prepare_threads (struct parasite_ctl *ctl
    ...
    list_for_each_entry(thread_item, &(item->threads), thread_list) {
		...
		struct pid *tid = &thread_item->thread;
...
struct parasite_ctl *parasite_infect_seized (pid_t pid, struct pstree_item *item
        , struct vm_area_list *vma_area_list, bool dump_pages_later) {
    ...
    BUG_ON(thread_item->thread.real != pid);
    ...
    // XXX: Shiv
    // Initialize arg size for various dump types.
    ctl->dain_fds_arg_size = 0;
    ctl->posix_timers_arg_size = 0;
    ctl->dump_pages_arg_size = 0;
    ctl->aio_rings_arg_size = 0;
    ctl->cgroup_arg_size = 0;
    ctl->thread_arg_size = 0;
    ctl->sigacts_arg_size = 0;
    ctl->itimers_arg_size = 0;
    ctl->misc_arg_size = 0;
    ctl->tty_arg_size = 0;
    ctl->args_offset = 0;

    if (dump_pages_later) {
        ctl->dump_pages_arg_size = dump_pages_args_size (vma_area_list);
        ctl->aio_rings_arg_size = aio_rings_args_size (vma_area_list);
        ctl->cgroup_arg_size = dump_cgroup_args_size ();
        ctl->thread_arg_size = dump_threads_args_size ();
        ctl->sigacts_arg_size = dump_sigacts_args_size ();
        ctl->itimers_arg_size = dump_itimers_args_size ();
        ctl->misc_arg_size = dump_misc_args_size ();
        ctl->tty_arg_size = dump_tty_args_size ();
    }
	parasite_ensure_args_size (dump_pages_args_size (vma_area_list), dump_pages_later);
	parasite_ensure_args_size (aio_rings_args_size (vma_area_list), dump_pages_later);
    ...
    //
    // In VANILA CRIU:
    // It is assumed that after dumping pages, aio ring will be dumped
    // independently which may overwrite the structures, i.e.
    //      parasite_ensure_args_size = max (dump_pages_args_size
    //      , aio_rings_args_size, cgroup_args_size, ...);
    // 
    // But in New CRIU:
    // I will make following semantics for pre-dump only:
    //      parasite_ensure_args_size = dump_pages_args_size
    //          + aio_rings_args_size + cgroup_args_size + thread_arg_size
    //          + ... + tty_arg_size;
    // 
    // And with this I have to also change how it gets args inside parasite
    // code to pre-dump various infos.
    //
	parasite_ensure_args_size (dump_cgroup_args_size (), dump_pages_later);
	parasite_ensure_args_size (dump_threads_args_size (), dump_pages_later);
	parasite_ensure_args_size (dump_sigacts_args_size (), dump_pages_later);
	parasite_ensure_args_size (dump_itimers_args_size (), dump_pages_later);
	parasite_ensure_args_size (dump_misc_args_size (), dump_pages_later);
	parasite_ensure_args_size (dump_tty_args_size (), dump_pages_later);
    // END
