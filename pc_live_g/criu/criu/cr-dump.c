...
/**
* XXX: Shiv
* Same reason as mentioned in cr_dump_tasks
*/
bool dump_aio_ring_flag = false;
...
static int dump_task_core_all (struct parasite_ctl *ctl, struct pstree_item *item
        , const struct proc_pid_stat *stat, const struct cr_imgset *cr_imgset
        , const struct parasite_dump_misc *misc, bool dump_cgroup_with_core) {
     ...
     */
    // Shiv:
    // Dump cgroup with cores only if it is given.
    if (!dump_cgroup_with_core) {
        // core->tc->has_cg_set = false; // TODO: Shiv: check whether it is needed.
        goto skip_cgroup_dumping;
    }
...
static int dump_task_thread (struct parasite_ctl *parasite_ctl
        , struct pstree_item *item, int id, struct pid *tid) {
    ...
    pstree_insert_pid_thread (item, tid);
...
static int dump_task_signals (pid_t pid, struct pstree_item *item) {
    ...
    list_for_each_entry(thread_item, &(item->threads), thread_list) {
        ...
        ret = dump_signal_queue (thread_item->thread.real, &item->core[i]->thread_core->signals_p, false);
        if (ret) {
            pr_err("Can't dump private signals for thread %d\n", thread_item->thread.real);
            return -1;
        }
...
static int dump_task_threads (struct parasite_ctl *parasite_ctl
    ...
    list_for_each_entry(thread_item, &(item->threads), thread_list) {
        ...
        if (item->pid->real == thread_item->thread.real) {
            thread_item->thread.ns[0].virt = vpid(item);
            continue;
        }
        if (dump_task_thread (parasite_ctl, item, i, &(thread_item->thread)))
            return -1;
...
static int pre_dump_one_task(struct pstree_item *item, InventoryEntry *parent_ie)
{
    ...
    ret = -1;
    // XXX: Shiv
    // Here I am passing dump arg as true to make Vanila CRIU pre-dump work
    // like previous design.
...
// Function to dump cgroup of a task.
static int pre_dump_cgroup_tasks_general (struct parasite_ctl *parasite_ctl
        , struct pstree_item *item) {

    // Shiv:
    // Copied from *dump_task_core_all* function.
    //
    // XXX:
    // The lable *dump_cgroup* must be below *dump_cores* (STRICT) because
    // dumping core will dump cgroup (according to Vanila CRIU implementation,
    // look at at function *dump_task_core_all*).
    // So, core and cgroup are kind of dependent.
    //
    struct parasite_dump_cgroup_args cgroup_args;
    struct parasite_dump_cgroup_args *info = NULL;
    int ret;
    int pid = item->pid->real;

    if (item->ids->has_cgroup_ns_id && !item->parent) {
        info = &cgroup_args;
        ret = parasite_dump_cgroup(parasite_ctl, &cgroup_args);
        if (ret)
            goto err;
    }

    ret = dump_task_cgroup(item, NULL, info);
    if (ret)
        goto err;
...
err:
    pr_debug ("Some error during dumping of cgroup for pid: %d\n", pid);
    return ret;
...
//
// TODO
// Shiv: Pre-dumping one task with all the given dump info files. It is called
// from "cr_pre_dump_tasks_general" function.
//
// The code is copied from dump_one_task function entirely and then modified.
//
// Start Date: 19th Dec 2021
//
static int pre_dump_one_task_general (struct pstree_item *item
    ...
    if (!cr_dinfo_bitmap [DUMP_FDINFO])
        goto prep_posix_timers;
    ...
    if (!shared_fdtable(item)) {
        ...
        parasite_ensure_args_size (drain_fds_size(dfds), true);
    }

prep_posix_timers:

    if (!cr_dinfo_bitmap [DUMP_POSIX_TIMERS])
        goto prep_signals;
    ...
    parasite_ensure_args_size (posix_timers_dump_size(proc_args.timer_n), true);

prep_signals:

    if (!cr_dinfo_bitmap[DUMP_SIGNALS])
        goto task_seize;
...
task_seize:
    ...
    //
    // XXX: Shiv
    // Set drain_fds and posix timers arg sizes for parasite ctl here because
    // you have got the ctl only here. But before this check whether it was
    // going to be dumped (with the --dump-info option).
    //
    // NOTE:
    // You have to do this in dump as well but there you will not check for
    // any condition because everything will be dumped.
    //
    if (cr_dinfo_bitmap[DUMP_POSIX_TIMERS]) {
        parasite_ctl->posix_timers_arg_size = (unsigned long) posix_timers_dump_size (proc_args.timer_n);
    }
    if (dfds != NULL) {     // Checking dfds is just sufficient, no need to check cr_dinfo_bitmap
        parasite_ctl->dain_fds_arg_size = (unsigned long) drain_fds_size (dfds);
    }
    ...
    if (!cr_dinfo_bitmap[DUMP_ZOMBIES])
        goto vdso;
...
dump_fdinfo:
    ...
    if (!cr_dinfo_bitmap[DUMP_PAGES_AND_MAP]) {
        goto dump_posix_timers;
    }
...
dump_posix_timers:
    ...
    // Shiv: Dump posix timer if it is given in the --dump-info option.
    if (!cr_dinfo_bitmap[DUMP_POSIX_TIMERS]) {
        goto dump_cores;
    }
...
dump_cores:
    ...
    ret = dump_task_core_all(parasite_ctl, item, &pps_buf, cr_imgset
            , &misc, cr_dinfo_bitmap[DUMP_CGROUP]);
    if (ret) {
...
dump_cgroup:
    ...
    if (!(cr_dinfo_bitmap [DUMP_CGROUP] && !cr_dinfo_bitmap[DUMP_CORE]))
        goto dump_mm;

    ret = pre_dump_cgroup_tasks_general (parasite_ctl, item);
    if (ret) {
...
err_cure_imgset:
    pr_debug ("Some fatal error which is going to cure remote and local now!!\n");
...
static int dump_one_task (struct pstree_item *item, InventoryEntry *parent_ie) {
    ...
    if (!shared_fdtable(item)) {
        ...
        parasite_ensure_args_size (drain_fds_size(dfds), dump_pages_later);
    }
    ...
    parasite_ensure_args_size (posix_timers_dump_size(proc_args.timer_n), dump_pages_later);
    ...
    parasite_ctl = parasite_infect_seized (pid, item, &vmas, dump_pages_later);
    if (!parasite_ctl) {
        pr_err("Can't infect (pid: %d) with parasite\n", pid);
        goto err;
    }
    ...
    mdc.pre_dump = true;        /* FIXME: Shiv: Workaround to dump pages later for dump round as well. */
    ...
    ret = dump_task_core_all(parasite_ctl, item, &pps_buf, cr_imgset, &misc, true);
    if (ret) {
        pr_err("Dump core (pid: %d) failed with %d\n", pid, ret);
        goto err_cure;
    }
...
err:
    ...
    total_sent_mbytes = ((total_sent_bytes) / (1024 * 1024));
    pr_info("Sent %ld (KB) to destination\n", (total_sent_bytes / 1024));
    pr_info("Sent %ld (MB) to destination\n", total_sent_mbytes);
    if (ret)
...
int cr_pre_dump_tasks(pid_t pid)
{
    ...
    if (memfd_inodes_init (false))
        goto err;

    if (file_lock_shalloc (false))
        goto err;

    if (ghost_files_shalloc (false))
        goto err;

    if (unix_sockets_shalloc (false))
        goto err;

    if (connect_to_xfer_server_to_send() < 0)
        goto err;
...
static int cr_dump_finish (int ret) {
     ...
     * XXX: Shiv
     *
     * Delete iptables rules added by CRIU, if it is not deleted by above
     * if block.
     */
    if (!network_unlocked) {
        network_unlock();
    ...
    // Indicate xfer-server that sender want to send pages and pagemap.
    if (init_page_transfer_xfer_server() < 0)
        goto cure_local;

    pr_info("Dumping tasks' memory\n");
    for_each_pstree_item(item) {
        struct parasite_ctl *ctl = dmpi(item)->parasite_ctl;
        struct page_pipe *mem_pp;
        struct page_xfer xfer;
...
err:
    ...
    if (disconnect_from_xfer_server())
        ret = -1;
    ...
    if (ret) {
        ...
        gettimeofday(&t, NULL);
        write_stats(DUMP_STATS);
        pr_info("Dumping finished successfully (timestamp: %ld.%ld)\n", t.tv_sec, t.tv_usec);
    }
...
int cr_dump_tasks(pid_t pid)
{
    ...
    /**
     * XXX: Shiv
     * Workaround to dump AIO ring to complete the implementation before SoCC'24 conf.
     * This is needed because in dump, I have mentioned mdc.pre_dump = true to transfer
     * pages at the end because of our design choices. Will revisit these later.
     */
    dump_aio_ring_flag = true;

    gettimeofday(&t, NULL);
    pr_info("Dump started (timestamp: %ld.%ld)\n", t.tv_sec, t.tv_usec);
    ...
    // XXX: Shiv
    //
    // First let's set all value of cr_dinfo_bitmap to 1, so wherever this is used
    // during pre-dump and dump both. It should not create any problem.
    //
    // Reason:
    // cr_dinfo_bitmap[DUMP_IDS] is used in collect_mnts function for pre-dump
    // and this function is also used for dump. And that is why I was getting
    // external mount error in restore with just one dump-info option
    // as "pages_and_map" during pre-dump and dump from command line.
    //
    for (int i = 0; i < DUMP_MAX_VALUE; i++) {
        cr_dinfo_bitmap [i] = 1;
    }
    ...
    if (prepare_inventory(&he, true))
        goto err;

    if (connect_to_xfer_server_to_send() < 0)
        goto err;

    // Now send dumped file by RunC (descriptors.json) to xfer-server.
    if (send_runc_descriptors_file_xfer_server() < 0)
        goto err;
...
err:
    ...
    gettimeofday(&t, NULL);
    pr_info("Dump finished (timestamp: %ld.%ld)\n", t.tv_sec, t.tv_usec);

    return ret;

}

static int dump_all_info (InventoryEntry *he) {

    int ret = -1;

    /* MNT namespaces are dumped after files to save remapped links */
    // Shiv: Dump mountpoints if it is given in the --dump-info option.
    if (!cr_dinfo_bitmap [DUMP_MOUNTPOINTS]) {
        goto dump_others;
    }
...
dump_pstree_label:
    ...
    if (!cr_dinfo_bitmap [DUMP_PSTREE]) {
        goto dump_shmem_label;
    }
...
dump_shmem_label:
    ...
    // Shiv: Dump other namespaces (utsns, netns, timens, ipc) if it is given in the --dump-info option.
    if (!cr_dinfo_bitmap [DUMP_OTHER_NS])
        goto dump_apparmor_label;
...
dump_apparmor_label:
    ...
    // Shiv: Dump apparmor if it is given in the --dump-info option.
    if (!cr_dinfo_bitmap [DUMP_APPARMOR])
        goto dump_cgroup_label;
...
dump_cgroup_label:
    ...
    // Shiv: Dump cgroup if it is given in the --dump-info option.
    if (!cr_dinfo_bitmap [DUMP_CGROUP])
        goto dump_external_unix_sk_label;

    ret = dump_cgroups();
    if (ret)
        goto err;

dump_external_unix_sk_label:

    // Shiv: Dump external unix socket and file entries if it is given in the
    // --dump-info option. Unix external sockets are dumped into "files.img".
    if (!cr_dinfo_bitmap [DUMP_FILES])
        goto dump_inventory_label;

    ret = fix_external_unix_sockets();
    if (ret)
        goto err;

dump_inventory_label:

    // Shiv: Same as Pre-dump.
    he->has_pre_dump_mode = true;
    he->pre_dump_mode = opts.pre_dump_mode;
...
static int cr_pre_dump_finish_general (int status, InventoryEntry *he) {

    struct pstree_item *item;
    int ret;
    bool already_cured;
    long total_sent_mbytes;
    bool restore_threads = true;
    ...
    /*
     * Restore registers for tasks only. The threads have not been
     * infected because core is not dumped in pre-dump rounds.
     * Therefore, the thread register sets have not been changed.
     */
    if (unlikely (cr_dinfo_bitmap [DUMP_CORE])) {
        restore_threads = true;
    }
    ret = arch_set_thread_regs(root_item, restore_threads);
    if (ret)
        goto err;
    ...
    // XXX: Shiv
    // Optimization to minimize the freeze time in pre-dump rounds.
    ret = dump_all_info (he);
    if (ret)
        goto err;

    close_cr_imgset(&glob_imgset);

    // Call cgp_finish if you have dumped cgroup.
    if (cr_dinfo_bitmap [DUMP_CGROUP])
        cgp_fini();

    // Check whether to --dinfo pages_and_map given in the option.
    // If yes, then dump otherwise compel cure local.
    already_cured = false;
...
err:
    ...
    // Shiv: Close this since it was opened by --dump-info option to dump to
    // "files.img" file.
    close_service_fd(CR_PROC_FD_OFF);
...
/**
 * Function to send dump-info bitmap to the xfer-server.
 * Return value:
 *      0   -> on success.
 *      -1  -> on some error.
 */
static inline int send_dinfo_bitmap_xfer_server (void) {

    int ret = -1;
    struct xfer_metadata xfer_meta;

    // If xfer-server option is not mentioned return simply.
    if (!opts.use_xfer_server)
        return 0;

    xfer_meta.type = CR_FD_XF_REQ_DINFO_BITMAP;
    xfer_meta.size = sizeof(cr_dinfo_bitmap);
    xfer_meta.img_name[0] = '\0';
    ret = send_to_xfer_server_buf (&xfer_meta, (void *)cr_dinfo_bitmap);

    return ret;

}

int cr_pre_dump_tasks_general (pid_t pid) {
    ...
    //int pre_dump_ret;
    int ret = -1;
    bool dump_cgroup_flag = false;

    pr_info("========================================\n");
    pr_info("Pre-dumping processes (pid: %d)\n", pid);
    pr_info("Var (num_dump_info: %d)\n", num_dump_info);
    pr_info("========================================\n");
    ...
    dump_cgroup_flag = (cr_dinfo_bitmap[DUMP_CGROUP]) ? true : false;
    if (!dump_cgroup_flag) {    // Shiv: If cgroup is not dumping go for cpuinfo dumping
        goto dump_cpuinfo;
    }
...
dump_cpuinfo:
    if (prepare_inventory(&he, dump_cgroup_flag))
        goto err;

    // XXX: Shiv
    // Connect to xfer-server before dumping anything on local .img file before
    // below info, i.e. cpuinfo
    if (connect_to_xfer_server_to_send() < 0)
        goto err;

    // First send dump-info bitmap to xfer-server.
    if (send_dinfo_bitmap_xfer_server() < 0)
        goto err;
    ...
    // Shiv: If entire ids are not dumping then just dump required ids for pre-dump (mnt and net).
    if (cr_dinfo_bitmap [DUMP_IDS]) {
     ...
     * XXX: Shiv
     *
     * Network should not be locked for pre-dump, right?
     *
     * Lock network when you are going to dump "fdinfo-%u.img" because it will
     * dump active tcp connections.
     *
    if (!cr_dinfo_bitmap [DUMP_FDINFO])
        goto skip_network_lock;
    */
...
//skip_network_lock:
    ...
    if (!cr_dinfo_bitmap [DUMP_SECCOMP]) {
        goto get_pe;
    }
...
get_pe:
    ...
    for_each_pstree_item(item) {
        if (pre_dump_one_task_general(item, parent_ie))
            goto err;
    }
    ...
    // Shiv: Saving iremap for next Pre-dump/ Dump.
    if (irmap_predump_prep())
        goto err;
...
err:
    ...
    return cr_pre_dump_finish_general(ret, &he);     // TODO: Shiv: Some checking is left inside this function.

}


// TODO:
// Shiv: Migrate function that calls pre-dump and dump in iterative fashion.
int cr_migrate_tasks (pid_t pid) {

    pr_debug ("Shiv: Tasks are going to be migrated. This is the entrypoint.\n");
    return 1;
