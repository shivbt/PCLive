...
static int setup_opts_from_req (int sk, CriuOpts *req) {
    ...
    // XXX: Shiv
    // Added to support parallel restore option flag with rpc.
    if (req->has_no_prestore) {
        opts.no_prestore = req->no_prestore;
    }

    // XXX: Shiv
    // Added to have iterations flag with rpc to test/ support parallel restore.
    if (req->has_iterations) {
        opts.iterations = req->iterations;
    }

    // XXX: Shiv
    // Added to have different restoration points with rpc to start parallel
    // restore for at different restoration points (iterations) during iterative
    // migration.
    if (req->has_rp) {
        opts.rp = req->rp;
    }
    ...
    // Shiv:
    // Same as above if construct with few modifications (xfer related).
	if (req->xfer) {

		opts.port = (short)req->xfer->port;

        // Since we are not supporting lazy pages and ps_socket fo xfer-server
        // , so remove it from this code.
        opts.use_xfer_server = true;
        if (req->xfer->address)
            SET_CHAR_OPTS(addr, req->xfer->address);
        else
            opts.addr = NULL;
	...
	for (i = 0; i < req -> n_dump_info; i++)
		if (add_dump_info (req->dump_info[i]))
			goto err;
...
int wait_for_go_signal_data (int *value) {

    int length;
    int fd, ret = 0;

    fd = get_service_fd (DATA_FIFO_PIPE_FD_OFF);
    if (fd < 0) {
        pr_perror ("Error: Can not find fd for DATA_FIFO_PIPE_FD_OFF\n");
	ret = -1;
	goto err;
    }
    if (read (fd, &length, sizeof(int)) != sizeof(int)) {
        pr_perror ("Error: Reading from data pipe failed!!!\n");
        ret = -1;
	length = -1;
	goto err;
    }
...
int wait_for_go_signal (int *virt_pid) {

    int pid = -1;
    int fd, ret = 0;

    fd = get_service_fd (FIFO_PIPE_FD_OFF);
    if (fd < 0) {
    ...
    if (read (fd, &pid, sizeof(int)) != sizeof(int)) {
        pr_perror ("Error: Reading from pipe failed!!!\n");
        ret = -1;
        pid = -1;
    }
...
static int restore_using_req (int sk, CriuOpts *req) {
    ...
    ////////////////////////////////////////////////////////////////////////////
    // TODO: Next Later
    // After testing remove below block
    // START:
    ////////////////////////////////////////////////////////////////////////////

    iterations = opts.iterations;
    // Check whether no of iterations are provided from command line or not.
    if (!iterations) {
        pr_perror ("Please provide no of iterations in case of parallel restore\n");
        goto exit;
    }

    // Open image root dir.
    images_dir_len = strlen(images_dir) - 2;
    strncpy(images_dir_path, images_dir, PATH_MAX);
    images_dir_path[images_dir_len] = '\0';
    ret = open_image_root_dir (images_dir_path);
    if (ret < 0) {
    ...
    // Create named fifo pipe to communicate with the xfer-server daemon.
    mkfifo (PRESTORE_PIPE_XFER_TO_CRIU_DATA, 0666);
    mkfifo (PRESTORE_PIPE_XFER_TO_CRIU_MSG, 0666);

    // XXX: Shiv
    // Open named pipe here so that container's processes can communicate with
    // xfer-server.
    ret = open_fifo_pipe_file (PRESTORE_PIPE_XFER_TO_CRIU_MSG, FIFO_PIPE_FD_OFF);
    if (ret < 0) {
        pr_err("Couldn't open provided named pipe dir %s\n", PRESTORE_PIPE_XFER_TO_CRIU_MSG);
        goto exit;
    }
    ret = open_fifo_pipe_file (PRESTORE_PIPE_XFER_TO_CRIU_DATA, DATA_FIFO_PIPE_FD_OFF);
    if (ret < 0) {
        pr_err("Couldn't open provided named data pipe dir %s\n", PRESTORE_PIPE_XFER_TO_CRIU_DATA);
        goto exit;
    }

    // Check whether restoration point is provided from command line (or RPC) or not.
    res_point = opts.rp;
    if (!res_point) {
        pr_perror ("Please provide restoration point in case of parallel restore\n");
        goto exit;
    }

    // Now perform the parallel or pipelined restore.
    res_iter = 1;
    for (i = res_point; i <= iterations; i++) {

        gettimeofday(&t, NULL);
        pr_debug ("%dth iteration is started (timestamp: %ld.%ld).\n", res_iter, t.tv_sec, t.tv_usec);
        ...
        // Restoration point (opts.rp) could be the last iteration, in that case
        // both init and last iter flags has to be true.
        if (res_point == iterations) {
            init_iter = true;
            last_iter = true;
        }

        // Now setup images dir to point "IMG_FD_OFF" to correct directory (iteratively).
        ret = setup_images_dir (i, true, false);
        if (ret < 0) {
            pr_perror ("Error while setting up image directory (%d)", i);
            goto exit;
        }

        // Finally call the pre-restore function.
	    if (cr_pre_restore_tasks(res_iter, init_iter, last_iter)) {
            pr_perror ("prestrore call returned Error in %dth iteration\n", res_iter);
		    goto exit;
        }

        gettimeofday(&t, NULL);
        pr_debug ("%dth iteration is over (timestamp: %ld.%ld).\n", res_iter, t.tv_sec, t.tv_usec);

        // Close the image directory used for this iteration.
        close_image_dir();

        // Now wait for signal from xfer-server to start next iteration.
        if (!last_iter && wait_for_go_signal (NULL)) {
            pr_perror ("Error while waiting for %dth iteration.\n", res_iter + 1);
            goto exit;
        }
        res_iter++;
    ...
    // Close image root dir and fifo data pipe.
    close_image_root_dir ();
    close_fifo_pipe_file (FIFO_PIPE_FD_OFF);
    close_fifo_pipe_file (DATA_FIFO_PIPE_FD_OFF);

    // Now delete the created named fifo pipe.
    unlink (PRESTORE_PIPE_XFER_TO_CRIU_MSG);
    unlink (PRESTORE_PIPE_XFER_TO_CRIU_DATA);
...
static int pre_dump_using_req_general (int sk, CriuOpts *req) {
    ...
    if (cr_pre_dump_tasks_general (req->pid))
        goto exit;
...
static int chk_keepopen_req(CriuReq *msg)
{
	...
	if (msg->type == CRIU_REQ_TYPE__PAGE_SERVER)
		...
		return 0;
	if (msg->type == CRIU_REQ_TYPE__XFER_SERVER)
		/* This just fork()-s so no leaks. Similiar to Page server */
		return 0;
...
more:
    ...
    /**
     * TODO: Shiv
     * Migrate command can be implemented by calling PRE_DUMP from runc in loop
     * using RPC and then calling DUMP from runc using RPC because PRE_DUMP
     * in CRIU is implemented using "pre_dump_loop" function which waits
     * in loop for successive pre-dump request from RPC and then final dump.
     * That is why see whether you need to actually implement migrate command
     * OR TAKE HELP FROM EXISTING CRIU SETUP??
     *
     * And if not then don't write below commented code and remove migrate
     * command setup from the CRIU but keep --prestore flag and remove --iterations
     * flag also since it won't be needed then.
     *
	case CRIU_REQ_TYPE__MIGRATE:
		ret = migrate_using_req(sk, msg->opts);
		break;
        */
	...
	case CRIU_REQ_TYPE__PRE_DUMP:
        // XXX: Shiv
        // Removed below line to pre-dump container once not in loop as in
        // Vanila CRIU.
		//ret = pre_dump_loop(sk, msg);
        ret = pre_dump_using_req_general (sk, msg->opts);
		break;
	case CRIU_REQ_TYPE__PAGE_SERVER:
		ret = start_page_server_req(sk, msg->opts, true);
		break;
	/* case CRIU_REQ_TYPE__XFER_SERVER:
		ret = start_xfer_server_req(sk, msg->opts, true);   // Shiv: function
		break;
    */
