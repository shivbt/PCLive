...
static struct hlist_head *file_desc_hash;
...
struct collect_image_info *files_cinfo = NULL;

int fdesc_shalloc (void) {

    file_desc_hash = fshmalloc (sizeof (*file_desc_hash) * FDESC_HASH_SIZE);
    if (file_desc_hash == NULL) {
...
void clean_saved_files (bool init_iter) {

    int i;
    struct file_desc *fd;

    // Return in case this is first iteration (init).
    if (init_iter)
        return ;

    pr_info("Cleaning File descs\n");
    for (i = 0; i < FDESC_HASH_SIZE; i++) {
        hlist_for_each_entry (fd, &file_desc_hash[i], hash) {
            struct fdinfo_list_entry *le, *te;
            list_for_each_entry_safe (le, te, &fd->fd_info_head, desc_list) {
                if (le->task->pid->is_deleted) {
                    list_del (&le->desc_list);
                    list_del (&le->ps_list);
                    fshfree_last (le);
                }
            }
...
static inline void __fshfree_fdinfo_entry_unpacked (FdinfoEntry *e, bool global_info) {
    ...
    fdinfo_entry__free_unpacked (e, alloc_ptr);
...
int prepare_fd_pid (struct pstree_item *item) {
    ...
    int curr_iter_item = item->curr_iter;
    bool global_info = true;

    if (unlikely (curr_iter_item == 1))
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &e, PB_FDINFO, global_info /*XXX: Later Check*/);
		if (ret <= 0)
			break;
        ...
        // Search whether this fdinfo is already added, if yes then skip collect.
        list_for_each_entry (fle, &rst_info->fds, ps_list) {
            if (fle->fe->fd == e->fd) {
                fd_is_found = true;
                __fshfree_fdinfo_entry_unpacked (e, global_info);
                break;
            }
        }
        if (fd_is_found)
...
int prepare_fs_pid(struct pstree_item *item)
{
	...
	ret = pb_read_one_eof(img, &fe, PB_FS, false);
	close_image(img);
	if (ret <= 0)
		goto out;
...
int shared_fdt_prepare(struct pstree_item *item)
{
	...
	if (!rsti(parent)->fdt) {
		fdt = fshmalloc(sizeof(*rsti(item)->fdt));
		if (fdt == NULL)
			return -1;
		...
		fdt->nr = 1;
		fdt->pid = vpid(parent);
        //pr_debug ("Shiv: This should be called JUST ONCE\n");
	} else
		fdt = rsti(parent)->fdt;
...
// Function to check whether file entry is already present from the previous
// iteration.
static int search_and_update_one_file (ProtobufCMessage *base) {

    // Since nothing is collected in the *collect_one_file*, so I will
    // return 0 from here. File entry is not collected with just one collect
    // call, it needs 2 collect calls (see *collect_one_file*).
    return 0;

}

static inline int init_files_cinfo (void) {

    files_cinfo = fshmalloc (sizeof (*files_cinfo));
    if (files_cinfo == NULL) {
    ...
    files_cinfo->search_and_update = search_and_update_one_file;
    files_cinfo->collect = collect_one_file;
    files_cinfo->flags = COLLECT_NOFREE;
    files_cinfo->info_type = PRST_INFO_GLOBAL;

    return 0;

}

static inline int init_or_reset_files (bool init_iter) {

    int ret = 1;
    // If it is the first iteration then do the initializations/ allocations first.
    if (init_iter) {
        ret = 0;
        init_fdesc_hash ();
        init_sk_info_hash ();
        ret = init_files_cinfo ();
        return ret;
    }

    // Remove collect happaned flag for files first.
    files_cinfo->flags &= ~COLLECT_HAPPENED;

    // TODO: Later
    // Check whether to reset the above flags for all other subsystems of files,
    // e.g. reg_file_cinfo etc.

    // TODO: Later
    // Mark every info as deleted before the processing for this iteration.
    // Because files.img is having one more level, i.e. file types that is
    // why marking for each subtype of files.img should be done just like
    // collect is done (collect is collect_entry for each file type differently).

    return 0;

}

static inline void clean_files (bool init_iter) {

    if (init_iter)
        return;

    // TODO: Later
    // Clean entries from the lists which was present in previous iteration but
    // not present in current iteration.

}

int prepare_files (bool init_iter) {

    int ret = -1;
    if (init_or_reset_files (init_iter))
        return ret;

    //
    // TODO: Later
    // Update for each subinfo is left to be implemented, I will implement this
    // when consumption of these update will be encountered while going ahead.
    //
    // Now collect the info.
    ret = collect_image(files_cinfo);
    clean_files (init_iter);
    return ret;
