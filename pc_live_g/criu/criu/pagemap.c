...
static int advance(struct page_read *pr) {
    ...
    if (!pr->is_pagemaps_cached) {

        pr->curr_pme = 0;
        pr->nr_pmes = 0;

try_read_again:
        ret = pb_read_one_eof(pr->pmi, &pr->pe, PB_PAGEMAP, false /*XXX: Later Check*/);
	if (ret == 0) {
		if (futex_get (&current->xfer_complete)) {
			num_retry++;
			if (num_retry > 2)
				return 0;
		}
        ...
        if (ret == 0)
            return 0;
        init_compat_pagemap_entry(pr->pe);
        pr->nr_pmes++;

        goto non_cached_read;
    }
...
static int read_parent_page (struct page_read *pr, struct vma_area *vma
        , unsigned long vaddr, int nr, void *buf, unsigned flags) {
	...
	do {
		...
		ret = ppr->read_pages(ppr, vma, vaddr, p_nr, buf, flags);
		if (ret == -1)
			return ret;
...
skip_sync:
	while (1) {
		...
		if (opts.auto_dedup && ret) {
			// Clean pages image file immediately after reading it in case of
			// parallel restore to optimize the memory consumption.
			int status;
			bool cleanup = opts.no_prestore ? false : true;
			status = punch_hole(pr, pr->pi_off + curr, ret, cleanup);
			if (status == -1)
				return -1;
		}
...
static int read_pagemap_page (struct page_read *pr, struct vma_area *vma
        , unsigned long vaddr, int nr, void *buf, unsigned flags) {
    ...
    // XXX: Shiv
    //
    // 1.) Read the pages directory from begin to end (iterative case), in
    //     case of parallel restore instead of end to begin and always read
    //     latest data.
    // 2.) Read in mixed way (both begin to end and end to begin) to implement
    //     variable restoration points like after 1st, 2nd, 5th etc.
    //
    // FIXME:
    // Currently not handeling lazy case here.
    if (!opts.no_prestore) {
        if (pagemap_present (pr->pe)) {
            ...
            if (!vma) {
                if (read_parent_page(pr, vma, vaddr, nr, buf, flags) < 0)
                    return -1;
            } else {
                unsigned long off;
                off = (vaddr - vma->e->start) / PAGE_SIZE;
                if (!test_bit(off, vma->page_bitmap)) {
                    if (read_parent_page(pr, vma, vaddr, nr, buf, flags) < 0)
                        return -1;
				}
            }
...
static int process_async_reads_mmap (struct page_read *pr) {
    ...
    list_for_each_entry_safe (piov, n, &pr->async, l) {
        ...
        void *src_mem = (void *)pr->pi->_x.b.mem;

        pr_debug("Read piov iovs %d, from %ju, len %ju, first %p:%zu\n", piov->nr, piov->from,
             piov->end - piov->from, piov->to->iov_base, piov->to->iov_len);

        // Copy all nr iovs form src with offset to the destination (to).
        src_mem += piov->from;
        for (i = 0; i < piov->nr; i++) {
            memcpy (piov->to[i].iov_base, src_mem, piov->to[i].iov_len);
            src_mem += piov->to[i].iov_len;
        }
    ...
    if (pr->parent) {
        ret = process_async_reads_mmap (pr->parent);
    }
...
static int process_async_reads(struct page_read *pr)
{
	...
	list_for_each_entry_safe(piov, n, &pr->async, l) {
		...
		pr_debug("Read piov iovs %d, from %ju, len %ju, first %p:%zu\n", piov->nr, piov->from,
			 piov->end - piov->from, piov->to->iov_base, piov->to->iov_len);
	more:
        prst_timing_start (TIME_PREADV);
		ret = preadv(fd, piov->to, piov->nr, piov->from);
        prst_timing_stop (TIME_PREADV);
	...
	// Truncate pages-%lu.img file to 0, if it is pipelined restore.
	if (!opts.no_prestore && opts.auto_dedup) {
		ret = ftruncate(img_raw_fd(pr->pi), 0);
		if (ret != 0) {
			pr_perror("Error in truncating");
			return -1;
		}
	}
...
static void close_page_read (struct page_read *pr) {
    ...
    // Unmap allotted space of mmapped region in case of --use-mmap flag.
    if (opts.use_mmap) {
        ret = munmap ((void *)pr->pi->_x.b.mem, pr->pi->_x.b.sz);
        if (ret < 0) {
            pr_perror ("Unable to unmap %p(%x)", (void *)pr->pi->_x.b.mem
                    , pr->pi->_x.b.sz);
        }
    }
...
static int try_open_parent(int dfd, unsigned long id, struct page_read *pr, int pr_flags)
{
    ...
    parent->is_pagemaps_cached = true;
...
static int init_pagemaps (struct page_read *pr) {
	...
	while (1) {
		int ret = pb_read_one_eof(pr->pmi, &pr->pmes[pr->nr_pmes], PB_PAGEMAP, false /*XXX: Later Check*/);
		if (ret < 0)
			goto free_pagemaps;
		if (ret == 0)
			break;
...
int open_page_read_at (int dfd, unsigned long img_id, struct page_read *pr, int pr_flags) {
	...
	pr->pmes = NULL;
	pr->pieok = false;
	pr->last_pipe_data = -2; // Values greater than or equals -1 is used for communication purpose for pRestore.

	pr->pmi = open_image_at(dfd, NULL, i_typ, O_RSTR, img_id);
	if (!pr->pmi)
		return -1;
    ...
    /**
     * XXX: Shiv
     * Don't open parent directory of pages in case we are going to start the
     * restoration from 1st iteration.
     */
    if (restoration_point <= 1)
        goto skip_parent_open;
...
skip_parent_open:
    ...
    if (opts.use_mmap) {
        pr->sync = process_async_reads_mmap;
    } else {
	...
	if (remote)
		...
		if (opts.no_prestore && !pr->parent && !opts.lazy_pages)
			pr->pieok = true;
	}
