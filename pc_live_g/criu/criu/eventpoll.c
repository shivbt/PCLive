...
// Function to check whether eventpoll tfd entry is already present from the
// previous iteration.
static int search_and_update_one_epoll_tfd (ProtobufCMessage *base) {
    ...
    tfde = pb_msg (base, EventpollTfdEntry);
    ...
    // Now search it.
    for (i = 0; i < efe->n_tfd; i++) {
        if (efe->tfd[i]->id == tfde->id) {
            memcpy (efe->tfd[i], tfde, sizeof (*tfde));
            return 1;
        }
    }
...
struct collect_image_info epoll_tfd_cinfo = {
    ...
    .search_and_update = search_and_update_one_epoll_tfd,
	.collect = collect_one_epoll_tfd,
	.flags = COLLECT_NOFREE,
};

// Function to check whether eventpoll file entry is already present from the
// previous iteration.
static int search_and_update_one_epoll (ProtobufCMessage *base) {
    ...
    efe = pb_msg (base, EventpollFileEntry);
    id = efe->id;
    type = (&desc_ops)->type;
...
struct collect_image_info epoll_cinfo = {
    ...
    .search_and_update = search_and_update_one_epoll,
