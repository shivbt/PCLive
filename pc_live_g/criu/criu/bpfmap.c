...
static struct bpfmap_data_rst **bpfmap_data_hash_table = NULL;

int bpfmap_data_ht_shalloc (void) {

    size_t size = sizeof (*bpfmap_data_hash_table) * BPFMAP_DATA_TABLE_SIZE;

    *bpfmap_data_hash_table = fshmalloc (sizeof (**bpfmap_data_hash_table));
    if (*bpfmap_data_hash_table == NULL)
        goto err;
    bpfmap_data_hash_table = fshmalloc (size);
    if (bpfmap_data_hash_table == NULL)
        goto err;

    memzero (bpfmap_data_hash_table, size);
    return 0;
...
// Function to check whether a bpfmapdata entry is already present from the
// previous iteration.
static int search_and_update_bpfmap_data (ProtobufCMessage *base) {
    ...
    bde = pb_msg (base, BpfmapDataEntry);
    table_index = bde->map_id & BPFMAP_DATA_HASH_MASK;

    // Now search it.
    for (map_data = bpfmap_data_hash_table [table_index]; map_data != NULL; map_data = map_data->next) {
        if (map_data->bde->map_id == bde->map_id) {
            memcpy (map_data->bde, bde, sizeof (*bde));
            return 1;
        }
    }
...
struct collect_image_info bpfmap_data_cinfo = {
    ...
    .search_and_update = search_and_update_bpfmap_data,
...
// Function to check whether a bpfmap entry is already present from the
// previous iteration.
static int search_and_update_one_bpfmap (ProtobufCMessage *base) {
    ...
    bpfe = pb_msg (base, BpfmapFileEntry);
    id = bpfe->id;
    type = (&bpfmap_desc_ops)->type;
...
struct collect_image_info bpfmap_cinfo = {
    ...
    .search_and_update = search_and_update_one_bpfmap,
