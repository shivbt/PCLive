...
int check_img_inventory (bool restore, bool init) {
...
InventoryEntry *get_parent_inventory(void)
{
	...
	img = open_image_at(dir, NULL, CR_FD_INVENTORY, O_RSTR);
	if (!img) {
...
int prepare_inventory (InventoryEntry *he, bool dump_cgroup_flag) {
    ...
    // Shiv:
    // Dump root cg set if cgroup.img is going to be dumped.
    if (!dump_cgroup_flag)
        goto skip_cgroup_dump;
...
struct cr_img *open_image_at(int dfd, char *img_name, int type
        , unsigned long flags, ...) {
...
skip_path:
    ...
    ret = do_open_image(img, dfd, type, oflags, path);
	if (ret) {
		close_image(img);
		return NULL;
	}
...
static int do_open_image (struct cr_img *img, int dfd, int type
    ...
    snprintf(img->name, NAME_MAX, "%s", path);
    img->ftype = type;
    ...
    // XXX: Shiv
    // Adding mmap and memcpy on tmpfs for pages-%d.img files.
    if (opts.use_mmap && img->ftype == CR_FD_PAGES) {

        if (mmap_pages_file(&img->_x))
            goto err;

    } else if ((oflags & O_NOBUF) || (img->ftype == CR_FD_PAGEMAP && prestore_started)){
...
void close_image (struct cr_img *img) {
    ...
    if (opts.use_mmap) {
		 ...
		 * data from it.
		 */
        //pr_debug("Shiv: This should not be printed in xfer-server log while closing\n");
...
/**
 * XXX:
 * It does not support finish of image-streaming like the mentioned function.
 */
void close_image_root_dir (void) {
	close_service_fd(IMG_ROOT_FD_OFF);
}

void close_fifo_pipe_file (int fd_off) {
	close_service_fd(fd_off);
}

/**
 * Shiv:
 * Same as "open_image_dir" with some modifications.
 * Return Value:
 *      0   -> on success.
 *      -1  -> on some error.
 */
int open_image_root_dir (char *dir) {

	int fd, ret;

	fd = open (dir, O_RDONLY);
	if (fd < 0) {
		pr_perror ("Can't open root dir %s", dir);
		return -1;
	}

	ret = install_service_fd (IMG_ROOT_FD_OFF, fd);
	if (ret < 0) {
		pr_perror ("install_service_fd for root dir failed.\n");
 ...
 *      -1  -> on some error.
 */
int open_fifo_pipe_file (char *dir, int fd_off) {

	int fd, ret;

	fd = open (dir, O_RDWR);
	if (fd < 0) {
		pr_perror ("Can't open data fifo pipe %s", dir);
		return -1;
	}

	ret = install_service_fd (fd_off, fd);
	if (ret < 0) {
		pr_perror ("install_service_fd for data fifo pipe failed.\n");
		close_fifo_pipe_file (fd_off);
		return -1;
	}
...
/**
 * Shiv:
 * Same as "open_image_dir" with some modifications. It does same but with fd
 * instead of pathname.
 * XXX:
 * It does not support image-streaming like "open_image_dir"
 * Return Value:
 *      0   -> on success.
 *      -1  -> on some error.
 */
int open_image_dir_at (int rfd, char *rel_path, bool check_parent) {

	int fd, ret;

	fd = openat (rfd, rel_path, O_RDONLY);
	if (fd < 0) {
		pr_perror("Can't open relative dir %s", rel_path);
		return -1;
	}
...
/**
 * Shiv:
 * Function to close all received dumped images using xfer-server.
 */
void close_all_images (void) {
...
struct cr_img *open_pages_image_at(int dfd, unsigned long flags, struct cr_img *pmi, u32 *id)
{
	...
	return open_image_at(dfd, NULL, CR_FD_PAGES, flags, *id);
}
...
/*
 * Send buffer @ptr of @size bytes to xfer-server.
 * Returns
 *      0   -> on success
 *      -1  -> on error (error message is printed)
 */
int write_img_buf_xfer (struct cr_img *img, const void *ptr, int size) {
    ...
    xfer_meta.size = size;
    xfer_meta.save_method = CR_IMG_BUF;
    snprintf(xfer_meta.img_name, NAME_MAX, "%s", img->name);
    ret = send_to_xfer_server_buf (&xfer_meta, ptr);

	if (ret < 0)
		pr_perror("Can't write img buffer to xfer-server");
	return ret;
