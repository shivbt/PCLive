...
static struct list_head *collected_ttys = NULL;
...
int collected_ttys_shalloc (void) {

    collected_ttys = fshmalloc (sizeof (*collected_ttys));
    if (collected_ttys == NULL) {
        pr_perror ("Can't allocate from fixed shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (collected_ttys);
    return 0;
...
{
	tty_mutex = fshmalloc(sizeof(*tty_mutex));
	if (!tty_mutex) {
 ...
 * XXX: Shiv
 *
 * Since tty info, data or anything related to tty is not dumped and sent in
 * each iteration, so we will simply return 0 from here with a design in mind
 * that it will be collected in the last iteration only.
 */
static int search_and_update_one_tty_info_entry (ProtobufCMessage *base) {
    return 0;
}
...
struct collect_image_info tty_info_cinfo = {
	...
	.pb_type = PB_TTY_INFO,
    .search_and_update = search_and_update_one_tty_info_entry,
 ...
 * that it will be collected in the last iteration only.
 */
static int search_and_update_one_tty (ProtobufCMessage *base) {
    return 0;
}
...
static int collect_one_tty (void *obj, ProtobufCMessage *msg, struct cr_img *i) {
    ...
    if (unlikely (collected_ttys == NULL))
...
struct collect_image_info tty_cinfo = {
    ...
    .search_and_update = search_and_update_one_tty,
 ...
 * that it will be collected in the last iteration only.
 */
static int search_and_update_one_tty_data (ProtobufCMessage *base) {
    return 0;
}
...
struct collect_image_info tty_cdata = {
    ...
    .search_and_update = search_and_update_one_tty_data,
