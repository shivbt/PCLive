...
static struct list_head *inet_ports = NULL;
...
int inet_ports_shalloc (void) {

    inet_ports = fshmalloc (sizeof (*inet_ports));
    if (inet_ports == NULL) {
        pr_perror ("Can't allocate from fixed shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (inet_ports);
    return 0;
...
// Function to check whether an Inet socket entry is already present from
// the previous iteration.
static int search_and_update_one_inetsk (ProtobufCMessage *base) {
    ...
    ie = pb_msg (base, InetSkEntry);
    id = ie->id;
    type = (&inet_desc_ops)->type;
...
struct collect_image_info inet_sk_cinfo = {
    ...
    .search_and_update = search_and_update_one_inetsk,
