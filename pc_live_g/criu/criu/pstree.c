...
struct pstree_item *__alloc_pstree_item (bool rst) {
    ...
    futex_init(&item->mem_notif);           // Added by Shiv for iterative parallel-restore.
    futex_init(&item->xfer_complete);           // Added by Shiv for iterative parallel-restore.
...
static void reset_pstree (void) {
	...
	futex_set (&item->xfer_complete, 0);
...
struct pstree_item *get_pstree_item_virt_iter (int virt_pid) {
    ...
    for_each_pstree_item (item) {
        if (item->pid->is_deleted)
            continue;
        if (item->pid->ns[0].virt == virt_pid)
            return item;
	}
    return NULL;
