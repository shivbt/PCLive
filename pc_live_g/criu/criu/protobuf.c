...
int do_pb_read_one (struct cr_img *img, void **pobj, int type, bool global_info
	...
	*pobj = NULL;
	...
	if (unlikely(empty_image(img)))
		...
		curr_off = img->_x.offset;
		if (is_xfer_process)
			goto skip_size_wait;

		while (!opts.no_prestore && img->ftype == CR_FD_PAGEMAP) {
			fsize = img_raw_size (img);
			if (((fsize - curr_off) >= sizeof(size)) || futex_get (&current->xfer_complete)) {
				break;
			}
			usleep (200);
		}
...
skip_size_wait:
	...
	if (is_xfer_process)
		goto skip_entry_wait;

	curr_off = img->_x.offset;
	while (!opts.no_prestore && img->ftype == CR_FD_PAGEMAP) {
		fsize = img_raw_size (img);
		if ((fsize - curr_off) >= size) {
			break;
		}
		usleep (500);
	}
