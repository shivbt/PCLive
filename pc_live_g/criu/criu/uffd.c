...
static MmEntry *init_mm_entry(struct lazy_pages_info *lpi)
{
	...
	ret = pb_read_one_eof(img, &mm, PB_MM, false /*XXX: Don't care about lazy*/);
	close_image(img);
	if (ret == -1)
		return NULL;
...
static int ud_open(int client, struct lazy_pages_info **_lpi)
{
	...
	if (opts.use_page_server)
		pr_flags |= PR_REMOTE;
    lpi->pr.is_pagemaps_cached = true;
...
static int uffd_handle_pages(struct lazy_pages_info *lpi, __u64 address, int nr, unsigned flags)
{
    ...
    /**
     * XXX: Shiv
     * Skipping uffd way of handling page faults to add into consideration for
     * the design of "restoration points" of evaluation section.
     */
	ret = lpi->pr.read_pages(&lpi->pr, NULL, address, nr, lpi->buf, flags);
	if (ret <= 0) {
