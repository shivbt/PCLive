...
// XXX: Shiv
// pCRIU version of above function which support multiple arg regions.
void *compel_parasite_args_s (struct parasite_ctl *ctl, int region_type) {

    unsigned long addr;
    unsigned long size = 0;

    BUG_ON ((region_type <= PARASITE_REGION_MIN) || (region_type >= PARASITE_REGION_MAX));

    switch (region_type) {
        case PARASITE_DRAIN_FDS_ARGS_REGION:
            size = 0;
            break;
        case PARASITE_POSIX_TIMERS_ARGS_REGION:
            size = ctl->dain_fds_arg_size;
            break;
        case PARASITE_DUMP_PAGES_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size;
            break;
        case PARASITE_AIO_RINGS_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size;
            break;
        case PARASITE_DUMP_CGROUP_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size + ctl->aio_rings_arg_size;
            break;
        case PARASITE_DUMP_THREAD_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size + ctl->aio_rings_arg_size
                    + ctl->cgroup_arg_size;
            break;
        case PARASITE_DUMP_SIGACTS_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size + ctl->aio_rings_arg_size
                    + ctl->cgroup_arg_size + ctl->thread_arg_size;
            break;
        case PARASITE_DUMP_ITIMERS_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size + ctl->aio_rings_arg_size
                    + ctl->cgroup_arg_size + ctl->thread_arg_size
                    + ctl->sigacts_arg_size;
            break;
        case PARASITE_DUMP_MISC_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size + ctl->aio_rings_arg_size
                    + ctl->cgroup_arg_size + ctl->thread_arg_size
                    + ctl->sigacts_arg_size + ctl->itimers_arg_size;
            break;
        case PARASITE_DUMP_TTY_ARGS_REGION:
            size = ctl->dain_fds_arg_size + ctl->posix_timers_arg_size
                    + ctl->dump_pages_arg_size + ctl->aio_rings_arg_size
                    + ctl->cgroup_arg_size + ctl->thread_arg_size
                    + ctl->sigacts_arg_size + ctl->itimers_arg_size
    ...
    addr = ((unsigned long)compel_parasite_args_p (ctl)) + size;
    /*pr_debug ("Shiv: compel_parasite_args_s function => init addr: %lu"
            ", actual addr: %lu"
            ", size: %lu\n"
            , ((unsigned long)compel_parasite_args_p (ctl))
            , addr
            , size);*/
    return ((void *)addr);
