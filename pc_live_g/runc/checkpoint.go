...
checkpointed.`,
	...
	Flags: []cli.Flag{
        ...
        // Added by Shiv
        cli.StringFlag{Name: "xfer-server", Value: "", Usage: "ADDRESS:PORT of the xfer server"},
        cli.StringSliceFlag{Name: "dump-info", Usage: "dump these infos while pre-dumping"},
        cli.BoolFlag{Name: "no-prestore", Usage: "don't do pipelined restore"},
        cli.BoolFlag{Name: "dup-processes", Usage: "duplicate container process tree"},
	},
...
func criuOptions(context *cli.Context) (*libcontainer.CriuOpts, error) {
	...
	opts := &libcontainer.CriuOpts{
        ...
        NoPrestore:              context.Bool("no-prestore"),
        Iterations:              int32(context.Int("iterations")),
        Rp:                      int32(context.Int("rp")),
        UseMmap:                 context.Bool("use-mmap"),
        DupProcesses:              context.Bool("dup-processes"),
	}
    ...
    // Added by Shiv for --xfer-server option.
	if xferOpt := context.String("xfer-server"); xferOpt != "" {
		address, port, err := net.SplitHostPort(xferOpt)

		if err != nil || address == "" || port == "" {
			return nil, errors.New("Use --xfer-server ADDRESS:PORT to specify xfer server")
		}
		portInt, err := strconv.Atoi(port)
		if err != nil {
			return nil, errors.New("Invalid port number")
		}
		opts.XferServer = libcontainer.CriuXferServerInfo{
			Address: address,
			Port:    int32(portInt),
		}
	}

    // Added by Shiv for --dump-info option.
    for _, dinfo := range context.StringSlice("dump-info") {
        opts.DumpInfo = append(opts.DumpInfo, dinfo)
	}
