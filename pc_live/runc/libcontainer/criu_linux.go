...
func (c *Container) Checkpoint(criuOpts *CriuOpts) error {
	...
	rpcOpts := criurpc.CriuOpts{
		...
		DupProcesses:      proto.Bool(criuOpts.DupProcesses),
	}
    ...
    // Shiv: Appending optional rclive opts like --xfer-server and --dump-info
    // START
    if criuOpts.XferServer.Address != "" && criuOpts.XferServer.Port != 0 {
		rpcOpts.Xfer = &criurpc.CriuXferServerInfo{
			Address: proto.String(criuOpts.XferServer.Address),
			Port:    proto.Int32(criuOpts.XferServer.Port),
		}
	}
    if criuOpts.DumpInfo != nil {
        for _, dinfo := range criuOpts.DumpInfo {
            rpcOpts.DumpInfo = append (rpcOpts.DumpInfo, dinfo)
        }
    }
    // END
    ...
    /**
     * XXX: Shiv
     * We will dump everything even if it is pre-dump round (as opposed to above).
     */
    // Start
    hasCgroupns := c.config.Namespaces.Contains(configs.NEWCGROUP)
    for _, m := range c.config.Mounts {
...
func (c *Container) Restore(process *Process, criuOpts *CriuOpts) error {
	...
	req := &criurpc.CriuReq{
		...
		Opts: &criurpc.CriuOpts{
			...
			AutoDedup:       proto.Bool(criuOpts.AutoDedup),
			LazyPages:       proto.Bool(criuOpts.LazyPages),
			NoPrestore:      proto.Bool(criuOpts.NoPrestore),
            Iterations:      proto.Int32(criuOpts.Iterations),
            Rp:              proto.Int32(criuOpts.Rp),
			UseMmap:         proto.Bool(criuOpts.UseMmap),
		},
	}
