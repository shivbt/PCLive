...
// TODO: Next
//
// Make it iterative. It will pass the info to prepare_mappings fn.
...
static int restore_updated_priv_vma_content (struct pstree_item *t
    ...
    while (1) {
        ...
        if (pagemap_in_parent (pr->pe)) {
            ...
            ret = pr->read_pages (pr, vma, va, nr_pages, p_new, PR_ASYNC);
	...
	while (1) {
		...
		for (i = 0; i < nr_pages; i++) {
			...
			if (vma_inherited(vma)) {
                ...
                prst_timing_start (TIME_READ);
				ret = pr->read_pages(pr, vma, va, nr, p, PR_ASYNC);
                prst_timing_stop (TIME_READ);
				if (ret < 0)
					goto err_read;
