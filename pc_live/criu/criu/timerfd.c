...
// Function to check whether a timerfd entry is already present from
// the previous iteration.
static int search_and_update_one_timerfd (ProtobufCMessage *base) {
    ...
    tfe = pb_msg (base, TimerfdEntry);
    id = tfe->id;
    type = (&timerfd_desc_ops)->type;
...
struct collect_image_info timerfd_cinfo = {
    ...
    .search_and_update = search_and_update_one_timerfd,
