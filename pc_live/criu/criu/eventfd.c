...
// Function to check whether a eventfd file entry is already present from the
// previous iteration.
static int search_and_update_one_efd (ProtobufCMessage *base) {
    ...
    efe = pb_msg (base, EventfdFileEntry);
    id = efe->id;
    type = (&eventfd_desc_ops)->type;
...
struct collect_image_info eventfd_cinfo = {
    ...
    .search_and_update = search_and_update_one_efd,
