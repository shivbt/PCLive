...
// Function to check whether an ns file entry is already present from
// the previous iteration.
static int search_and_update_one_nsfile (ProtobufCMessage *base) {

    struct ns_file_info *nfi;
    struct file_desc *d;
    NsFileEntry *nfe;
    unsigned int id;
    int type;
    ...
    nfe = pb_msg (base, NsFileEntry);
    id = nfe->id;
    type = (&ns_desc_ops)->type;

    // Search for this id and type.
    d = find_file_desc_raw (type, id);
    if (d != NULL) {
        nfi = container_of (d, struct ns_file_info, d);
        nfi->nfe = nfe;
        return 1;
    }
...
struct collect_image_info nsfile_cinfo = {
    ...
    .search_and_update = search_and_update_one_nsfile,
...
static int read_pid_ns_img (void) {
	...
	for (ns = ns_ids; ns != NULL; ns = ns->next) {
		...
		ret = pb_read_one_eof(img, &e, PB_PIDNS, false /*XXX: Later Check*/);
		close_image(img);
		if (ret < 0) {
		...
		if (ret > 0)
			ns->ext_key = e->ext_key;   // XXX: Shiv -> Skipping the memory leak here in case of pRestore
	}
...
int prepare_namespace_before_tasks (bool init_iter) {

    // If it is first migration iteration then do the initializations otherwise
    // go to the image processing of different namespaces.
    if (!init_iter)
        goto read_ns_img;
