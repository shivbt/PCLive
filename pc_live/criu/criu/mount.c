 ...
 */
struct mount_info *mntinfo = NULL;
...
static int collect_mnt_from_image(struct mount_info **head, struct mount_info **tail, struct ns_id *nsid)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &me, PB_MNT, false /*XXX: Later Check*/);
		if (ret <= 0)
			break;
...
int read_mnt_ns_img (void) {
    ...
    // Delete mntinfo data processed in previous iteration.
    if (mntinfo) {

        // TODO: Shiv
        // Check whether this subroutine delete the list considering it as
        // circular linked list or simple null terminated single linked list.
        free_mntinfo (mntinfo);
...
static int collect_mntns(struct ns_id *ns, void *__arg)
{
     ...
     * XXX: Shiv
     * 
     * Go for root_item->id dereferencing if we have dumped ids which is given
     * from the command line option. That is why added extra check.
     *
     */
	if (arg->need_to_validate && cr_dinfo_bitmap[DUMP_IDS]
