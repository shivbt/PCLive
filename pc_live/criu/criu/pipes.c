...
static struct list_head *pipes = NULL;

int pipes_shalloc (void) {

    pipes = fshmalloc (sizeof (*pipes));
    if (pipes == NULL) {
...
static struct pipe_data_rst **pd_hash_pipes = NULL;

int pd_hash_pipes_shalloc (void) {

    size_t size = sizeof (*pd_hash_pipes) * PIPE_DATA_HASH_SIZE;
    pd_hash_pipes = fshmalloc (size);
    if (pd_hash_pipes == NULL)
        goto err;

    memzero (pd_hash_pipes, size);
    return 0;
...
// Function to check whether a pipe entry is already present from the
// previous iteration.
static int search_and_update_one_pipe (ProtobufCMessage *base) {

    struct pipe_info *pi;
    PipeEntry *pe;

    // Decode the image.
    pe = pb_msg (base, PipeEntry);

    // Search for this id.
    if (likely (pipes != NULL)) {
        list_for_each_entry (pi, pipes, list) {
            if (pi->pe->id == pe->id && pi->pe->pipe_id == pe->pipe_id) {
                pi->pe = pe;
                return 1;
            }
        }
...
struct collect_image_info pipe_cinfo = {
    ...
    .search_and_update = search_and_update_one_pipe,
	.collect = collect_one_pipe,
    .info_type = PRST_INFO_GLOBAL,
};

// Function to check whether a pipe data entry is already present from the
// previous iteration.
int search_and_update_pipe_data (ProtobufCMessage *base) {
    ...
    // Now search it.
    for (pd = pd_hash_pipes [table_index]; pd != NULL; pd = pd->next) {
...
struct collect_image_info pipe_data_cinfo = {
    ...
    .search_and_update = search_and_update_pipe_data,
