...
int irmap_predump_run(void)
{
	...
	img = open_image_at(AT_FDCWD, NULL, CR_FD_IRMAP_CACHE, O_DUMP);
	if (!img)
		return -1;
...
in:
	*img = open_image_at(dir, NULL, CR_FD_IRMAP_CACHE, O_RSTR);
	if (dir != AT_FDCWD)
		close(dir);
...
int irmap_load_cache(void)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &ic, PB_IRMAP_CACHE, false /*XXX: Later Check*/);
		if (ret <= 0)
			break;
