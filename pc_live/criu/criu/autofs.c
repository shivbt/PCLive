...
static int dup_pipe_info(struct pipe_info *pi, int flags, struct file_desc_ops *ops)
{
	...
	new = fshmalloc(sizeof(*new));
	if (!new)
		return -1;

	pe = fshmalloc(sizeof(*pe));
	if (!pe)
		return -1;
					 ...
					 const autofs_info_t *info)
{
	...
	i = fshmalloc(sizeof(*i));
	if (!i)
		return NULL;

	i->mnt_path = fshmalloc(strlen(mi->ns_mountpoint) + 1);
	if (!i->mnt_path)
		return NULL;
	 ...
	 * See autofs_mount().
	 */
	i->entry = fshmalloc(sizeof(*info->entry));
	if (!i->entry)
		return NULL;
...
static int autofs_restore_entry(struct mount_info *mi, AutofsEntry **entry)
{
	...
	ret = pb_read_one_eof(img, entry, PB_AUTOFS, false /*Later Check*/);
