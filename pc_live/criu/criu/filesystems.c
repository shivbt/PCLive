...
struct list_head *binfmt_misc_list = NULL;

int binfmt_misc_shalloc (void) {

    binfmt_misc_list = fshmalloc (sizeof (*binfmt_misc_list));
    if (binfmt_misc_list == NULL) {
        pr_perror ("Can't allocate more shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (binfmt_misc_list);
    return 0;
...
static int binfmt_misc_restore(struct mount_info *mi)
{
	...
	while (ret == 0) {
		...
		ret = pb_read_one_eof(img, &bme, PB_BINFMT_MISC, true /*XXX: Later Check*/);
		if (ret <= 0)
			break;
...
// Function to check whether Binfmt Misc entry is already present from the
// previous iteration.
static int search_and_update_one_binfmt_misc_entry (ProtobufCMessage *base) {

    BinfmtMiscEntry *bme;

    // Get the entry from protobuf.
    bme = pb_msg (base, BinfmtMiscEntry);
    ...
    if (!list_empty(binfmt_misc_list)) {
        ...
        struct binfmt_misc_info *bmi;
        list_for_each_entry (bmi, binfmt_misc_list, list) {
            if (strcmp(bmi->bme->name, bme->name)) {
                bmi->bme = bme;
                return 1;
            }
        }
...
struct collect_image_info binfmt_misc_cinfo = {
    ...
    .search_and_update = search_and_update_one_binfmt_misc_entry,
...
static int tmpfs_dump(struct mount_info *pm)
{
    ...
    // Send this file (tmpfs-dev-%u.tar.gz.img) to xfer-server if option is
    // specified.
    if ((ret == 0) && (opts.use_xfer_server)) {
        ...
        xfer_meta.size = (u32) img_raw_size(img);
        xfer_meta.save_method = CR_IMG_RAW;
        snprintf(xfer_meta.img_name, NAME_MAX, "%s", img->name);
        ret = send_to_xfer_server_fd (&xfer_meta, img_raw_fd(img));
	    close_image(img);
