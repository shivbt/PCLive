...
static struct list_head *unix_sockets = NULL;
static struct list_head *unix_ghost_addr = NULL;
...
int unix_ghosts_shalloc (void) {

    unix_ghost_addr = fshmalloc (sizeof (*unix_ghost_addr));
    if (unix_ghost_addr == NULL) {
        pr_perror ("Can't allocate from fixed shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (unix_ghost_addr);
    return 0;

}

int unix_sockets_shalloc (bool is_restore) {

    if (is_restore)
        unix_sockets = fshmalloc (sizeof (*unix_sockets));
    else
        unix_sockets = xmalloc (sizeof (*unix_sockets));
...
static int unix_collect_one (const struct unix_diag_msg *m, struct nlattr **tb
        , struct ns_id *ns) {
...
static struct hlist_head *sk_info_hash;

int skinfo_shalloc (void) {

    sk_info_hash = fshmalloc (sizeof (*sk_info_hash) * SK_INFO_HASH_SIZE);
    if (sk_info_hash == NULL) {
...
int unix_prepare_root_shared (void) {
    ...
    // Return if container does not have any unix sockets.
    if (unix_ghost_addr == NULL) {
        return 0;
    }

	mutex_ghost = fshmalloc(sizeof(*mutex_ghost));
	if (!mutex_ghost) {
...
// Function to check whether a unix socket entry is already present from the
// previous iteration.
static int search_and_update_one_unixsk (ProtobufCMessage *base) {

    struct unix_sk_info *ui;
    UnixSkEntry *ue;

    // Decode the image.
    ue = pb_msg (base, UnixSkEntry);

    // Search for this id.
    if (likely (unix_sockets != NULL)) {
        list_for_each_entry (ui, unix_sockets, list) {
            if (ui->ue->id == ue->id && ui->ue->ino == ue->ino && ui->ue->peer == ue->peer) {
                ui->ue = ue;
                return 1;
            }
        }
...
static int collect_one_unixsk (void *o, ProtobufCMessage *base, struct cr_img *i) {
    ...
    if (unlikely (unix_ghost_addr == NULL))
...
struct collect_image_info unix_sk_cinfo = {
    ...
    .search_and_update = search_and_update_one_unixsk,
...
static int add_fake_queuer(struct unix_sk_info *ui)
{
	...
	peer = fshmalloc (sizeof(struct unix_sk_info) + sizeof(UnixSkEntry) + sizeof(SkOptsEntry) + sizeof(FownEntry));
	if (peer == NULL)
		return -1;
...
int add_fake_unix_queuers (void) {
    ...
    // Return if no unix sockets are there for the container.
    if (unix_sockets == NULL) {
        return 0;
    }
