...
int setup_images_dir (int iteration, bool is_restore, bool check_parent) {
    ...
    if (mkdirat (fd, rel_img_path, 0766)) {
...
static inline int __start_prestore_direct (void) {
    ...
    strcpy(prestore_cmd[i++], opts.container_name);
    prestore_cmd[i] = NULL;
 ...
 * write 1 to give go signal to the main task which is waiting for it after
 * completing the processing of previous iteration.
 *
 * So, the creation and deletion of named fifo pipe is the responsibility of
 * parallel restore's main task.
 */
static inline int __signal_prestore (void) {
    ...
    while (!done) {
	if (access(PRESTORE_PIPE_XFER_TO_CRIU, F_OK) == 0)
	    done = 1;
	usleep (500);
    }

    fd = open (PRESTORE_PIPE_XFER_TO_CRIU, O_WRONLY);
    if (fd < 0) {
...
int cr_xfer_server () {
    ...
    for (i = 0; i < opts.iterations; i++) {
        ...
        // Skip parallel restore if it is specified in the command line option.
        if (opts.no_prestore || (i + 1) < opts.rp)
            continue;
