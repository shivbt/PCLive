...
static inline int __wait_for_go_signal (void) {

    char buf[2];
    int fd, ret = 0;
    fd = open (PRESTORE_PIPE_XFER_TO_CRIU, O_RDONLY);
    if (fd < 0) {
...
static int restore_using_req (int sk, CriuOpts *req) {
    ...
    mkfifo (PRESTORE_PIPE_XFER_TO_CRIU, 0666);
	...
	gettimeofday(&t, NULL);
        ...
        if (!last_iter && __wait_for_go_signal ()) {
