...
static inline void __orchestrate_task_tree (struct list_head *prst_list, int cmd) {
...
static int orchestrate_task_tree_changes (bool is_last_iter) {
    ...
    __orchestrate_task_tree (&prst_killed_tasks, PRST_EXIT);

    // Now orchestrate changes happened in current iteration.
    __prst_send_command_sync (root_item, PRST_UPDATE);

    // Now free the killed list.
    __prst_free_tasks_list (&prst_killed_tasks);

    // XXX: Shiv
    // Don't know why I have put return 0 here.
    return 0;
