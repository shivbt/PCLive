...
struct vma_area {
	...
	union {
		...
		struct /* for restore */ {
            ...
            unsigned long old_premmaped_addr;   // pRestore only
...
extern struct vma_area *alloc_vma_area (bool is_restore);
...
static inline struct vma_area *vma_prev (struct vma_area *vma) {
    return list_entry (vma->list.prev, struct vma_area, list);
}
