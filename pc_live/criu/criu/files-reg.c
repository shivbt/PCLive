...
static struct list_head *ghost_files = NULL;
...
{
	remap_open_lock = fshmalloc(sizeof(*remap_open_lock));
	if (!remap_open_lock)
		return -1;
...
static struct list_head *remaps = NULL;
static LIST_HEAD (remaps_rlb);      // Added for iterative prestore support.
...
static inline int remaps_shalloc (void) {

    remaps = fshmalloc (sizeof (*remaps));
    if (remaps == NULL) {
        pr_perror ("Can't allocate from fixed shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (remaps);
    return 0;

}

int ghost_files_shalloc (bool is_restore) {

    if (is_restore)
        ghost_files = fshmalloc (sizeof (*ghost_files));
    else
        ghost_files = xmalloc (sizeof (*ghost_files));
...
static int copy_file_from_chunks(struct cr_img *img, int fd, size_t file_size)
{
	...
	while (1) {
		...
		ret = pb_read_one_eof(img, &ce, PB_GHOST_CHUNK, false /*XXX: Later Check*/);
		if (ret <= 0)
			return ret;
...
static int collect_remap_ghost (struct reg_file_info *rfi, RemapFilePathEntry *rpe) {
	 ...
	 * try_clean_remaps()).
	 */
	gf->remap.rpath = fshmalloc(PATH_MAX);
	if (!gf->remap.rpath)
		return -1;
	gf->remap.rpath[0] = 0;
	gf->id = rpe->remap_id;
...
// Function to check whether remap file entry is already present from the
// previous iteration.
static int search_and_update_one_remap (ProtobufCMessage *base) {

    struct remap_info *ri;
    RemapFilePathEntry *rpe;

    // Decode the image.
    rpe = pb_msg (base, RemapFilePathEntry);

    // Now search.
    if (likely (remaps != NULL)) {
        list_for_each_entry (ri, remaps, list) {
            if ((ri->rpe->orig_id == rpe->orig_id) && (ri->rpe->remap_id == rpe->remap_id)) {
                ri->already_prepared = true;
                ri->rpe->has_remap_type = rpe->has_remap_type;
                ri->rpe->remap_type = rpe->remap_type;
                return 1;
            }
        }
    }
...
int prepare_remaps (bool is_init_iter) {

	struct remap_info *ri;
	int ret = 0;

    if (!is_init_iter)
        goto skip_init_remap;
...
skip_init_remap:
	list_for_each_entry(ri, remaps, list) {
        ...
        // Skip already prepared remap from previous iteration.
        if (ri->already_prepared)
            continue;
...
static struct collect_image_info remap_cinfo = {
    ...
    .search_and_update = search_and_update_one_remap,
...
static void __rollback_link_remaps(bool do_unlink)
{
	...
	list_for_each_entry_safe(rlb, tmp, &remaps_rlb, list) {
		if (do_unlink) {
...
static int open_filemap(int pid, struct vma_area *vma)
{
	...
	if (ctx.flags != flags || ctx.desc != vma->vmfd) {
		if (vma->e->status & VMA_AREA_MEMFD)
			ret = memfd_open(vma, vma->vmfd, &flags);
		else
...
// Function to check whether regular file entry is already present from the
// previous iteration.
static int search_and_update_one_regfile (ProtobufCMessage *base) {
    ...
    rfe = pb_msg (base, RegFileEntry);
    id = rfe->id;
    type = (&reg_desc_ops)->type;

    // Search for this id and type.
    d = find_file_desc_raw (type, id);
    if (d != NULL) {

        dot = fshmalloc (sizeof (char) * 2);
        if (dot == NULL)
            return 0;
...
struct collect_image_info reg_file_cinfo = {
    ...
    .search_and_update = search_and_update_one_regfile,
	.collect = collect_one_regfile,
	.flags = COLLECT_SHARED,
    .info_type = PRST_INFO_GLOBAL,
};

static inline int init_or_reset_remaps (bool init_iter) {

    // If it is the first iteration then first initialize the lists.
    if (init_iter) {

        if (remaps_shalloc ())
            return 1;
        if (ghost_files_shalloc (true /*Restore falg*/))
            return 1;
        return 0;

    }

    remap_cinfo.flags &= ~COLLECT_HAPPENED;

    // TODO: Later
    // Mark every info as deleted before the processing for iterations > 1.

    return 0;

}

static inline void clean_remaps (bool init_iter) {
...
int collect_remaps_and_regfiles (bool init_iter) {

	if (!files_collected() && collect_image(&reg_file_cinfo))
		return -1;

    // Init / Reset
    if (init_or_reset_remaps (init_iter))
        return -1;
