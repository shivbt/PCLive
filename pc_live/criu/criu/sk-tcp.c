...
static struct list_head *rst_tcp_repair_sockets = NULL;

int tcp_repair_sk_shalloc (void) {

    rst_tcp_repair_sockets = fshmalloc (sizeof (*rst_tcp_repair_sockets));
    if (rst_tcp_repair_sockets == NULL) {
        pr_perror ("Can't allocate from fixed shared space\n");
        return 1;
    }

    INIT_LIST_HEAD (rst_tcp_repair_sockets);
    return 0;
...
static int dump_tcp_conn_state(struct inet_sk_desc *sk)
{
	...
	if (buf) {
        if (opts.use_xfer_server) {
            ret = write_img_buf_xfer(img, buf, tse.inq_len);
        } else {
	...
	if (buf) {
        if (opts.use_xfer_server) {
		    ret = write_img_buf_xfer(img, buf, tse.outq_len);
        } else {
...
int prepare_tcp_socks (struct task_restore_args *ta) {
	...
	list_for_each_entry(ii, rst_tcp_repair_sockets, rlist) {
        ...
        // XXX: Shiv
        // Only add socket for this thread. This is needed because of sharing
        // nature of entire fdesc-s data among all processes.
        fdesc = &ii->d;
...
int tcp_locked_conn_add (struct inet_sk_info *ii) {

    if (unlikely (rst_tcp_repair_sockets == NULL)) {
