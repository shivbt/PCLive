...
int arch_set_thread_regs (struct pstree_item *item, bool with_threads) {
	...
	for_each_pstree_item(item) {
        ...
        list_for_each_entry(thread_item, &(item->threads), thread_list) {
            ...
            i++;    // This is because of continue keywords written below.

            //if (item->threads[i].state == TASK_DEAD || item->threads[i].state == TASK_ZOMBIE)
            thread = thread_item->thread;
            if (thread.state == TASK_DEAD || thread.state == TASK_ZOMBIE)
				continue;
			if (!with_threads && i > 0)
				continue;
			//if (set_task_regs(item->threads[i].real, item->core[i])) {
            if (set_task_regs(thread.real, item->core[i])) {
				pr_perror("Not set registers for task %d", thread.real);
				return -1;
			}
