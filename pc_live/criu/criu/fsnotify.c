...
// Function to check whether inotify file entry is already present from the
// previous iteration.
static int search_and_update_one_inotify (ProtobufCMessage *base) {
    ...
    ife = pb_msg (base, InotifyFileEntry);
    id = ife->id;
    type = (&inotify_desc_ops)->type;
...
struct collect_image_info inotify_cinfo = {
    ...
    .search_and_update = search_and_update_one_inotify,
...
// Function to check whether fanotify file entry is already present from the
// previous iteration.
static int search_and_update_one_fanotify (ProtobufCMessage *base) {
    ...
    // Decode the image and get id & type.
    ffe = pb_msg (base, FanotifyFileEntry);
    id = ffe->id;
    type = (&fanotify_desc_ops)->type;
...
struct collect_image_info fanotify_cinfo = {
	...
	.priv_size = sizeof(struct fsnotify_file_info),
    .search_and_update = search_and_update_one_fanotify,
	.collect = collect_one_fanotify,
    .info_type = PRST_INFO_GLOBAL,
};

// Function to check whether inotify mark entry is already present from the
// previous iteration.
static int search_and_update_one_inotify_mark (ProtobufCMessage *base) {

    InotifyWdEntry *iwe;
    struct file_desc *d;
    struct fsnotify_file_info *fi;
    struct fsnotify_mark_info *mi;

    if (!deprecated_ok("separate images for fsnotify marks"))
        return -1;

    iwe = pb_msg (base, InotifyWdEntry);
    d = find_file_desc_raw (FD_TYPES__INOTIFY, iwe->id);
    if (!d) {
        pr_err ("Can't find inotify with id %#08x\n", iwe->id);
        return -1;
    }

    fi = container_of (d, struct fsnotify_file_info, d);

    // Now search it.
    if (!list_empty (&fi->marks)) {
        list_for_each_entry (mi, &fi->marks, list) {
            if (mi->iwe->wd == iwe->wd) {
                memcpy (mi->iwe, iwe, sizeof (*iwe));
                return 1;
            }
        }
    }
...
struct collect_image_info inotify_mark_cinfo = {
    ...
    .search_and_update = search_and_update_one_inotify_mark,
	.collect = collect_one_inotify_mark,
};

// Function to check whether inotify mark entry is already present from the
// previous iteration.
static int search_and_update_one_fanotify_mark (ProtobufCMessage *base) {
    ...
    fme = pb_msg (base, FanotifyMarkEntry);
    d = find_file_desc_raw (FD_TYPES__FANOTIFY, fme->id);
    if (!d) {
        pr_err ("Can't find inotify with id %#08x\n", fme->id);
        return -1;
    }
...
struct collect_image_info fanotify_mark_cinfo = {
	...
	.priv_size = sizeof(struct fsnotify_mark_info),
    .search_and_update = search_and_update_one_fanotify_mark,
