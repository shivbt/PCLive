...
char *rpc_cfg_file;
int num_dump_info;
int restoration_point = 0;

int cr_dinfo_bitmap [DUMP_MAX_VALUE];
struct cr_img ***dump_imgs;
...
// Function from -> cr_options.h
int add_dump_info (char *dump_info) {

    int error = 1;
    if (!strcmp(dump_info, PAGES_AND_MAP_INFO)) {
        cr_dinfo_bitmap [DUMP_PAGES_AND_MAP] = 1;
        error = 0;
    } else if (!strcmp(dump_info, CGROUP_INFO)) {
        cr_dinfo_bitmap [DUMP_CGROUP] = 1;
        error = 0;
    } else if (!strcmp(dump_info, CORE_INFO)) {
        cr_dinfo_bitmap [DUMP_CORE] = 1;
        error = 0;
    } else if (!strcmp(dump_info, CPUINFO_INFO)) {
        cr_dinfo_bitmap [DUMP_CPUINFO] = 1;
        error = 0;
    } else if (!strcmp(dump_info, FDINFO_INFO)) {
        cr_dinfo_bitmap [DUMP_FDINFO] = 1;
        error = 0;
    } else if (!strcmp(dump_info, FILES_INFO)) {
        cr_dinfo_bitmap [DUMP_FILES] = 1;
        error = 0;
    } else if (!strcmp(dump_info, FS_INFO)) {
        cr_dinfo_bitmap [DUMP_FS] = 1;
        error = 0;
    } else if (!strcmp(dump_info, IDS_INFO)) {
        cr_dinfo_bitmap [DUMP_IDS] = 1;
        error = 0;
    } else if (!strcmp(dump_info, MM_INFO)) {
        cr_dinfo_bitmap [DUMP_MM] = 1;
        error = 0;
    } else if (!strcmp(dump_info, MOUNTPOINTS_INFO)) {
        cr_dinfo_bitmap [DUMP_MOUNTPOINTS] = 1;
        error = 0;
    } else if (!strcmp(dump_info, PSTREE_INFO)) {
        cr_dinfo_bitmap [DUMP_PSTREE] = 1;
        error = 0;
    } else if (!strcmp(dump_info, SECCOMP_INFO)) {
        cr_dinfo_bitmap [DUMP_SECCOMP] = 1;
        error = 0;
    } else if (!strcmp(dump_info, OTHER_NS_INFO)) {
        cr_dinfo_bitmap [DUMP_OTHER_NS] = 1;
        error = 0;
    } else if (!strcmp(dump_info, POSIX_TIMERS_INFO)) {
        cr_dinfo_bitmap [DUMP_POSIX_TIMERS] = 1;
        error = 0;
    } else if (!strcmp(dump_info, SIGNALS_INFO)) {
        cr_dinfo_bitmap [DUMP_SIGNALS] = 1;
        error = 0;
    } else if (!strcmp(dump_info, ZOMBIES_INFO)) {
        cr_dinfo_bitmap [DUMP_ZOMBIES] = 1;
        error = 0;
    } else if (!strcmp(dump_info, APPARMOR_INFO)) {
        cr_dinfo_bitmap [DUMP_APPARMOR] = 1;
        error = 0;
    }

    // Incremet the count to track no fo dump infos given.
    num_dump_info = (!error) ? (num_dump_info + 1) : num_dump_info;
...
void init_opts (void) {
    ...
    memset(cr_dinfo_bitmap, 0, sizeof(cr_dinfo_bitmap));
    for (i = 0; i < DUMP_MAX_VALUE; i++)
       cr_dinfo_bitmap [i] = 0;
    num_dump_info = 0;
...
#define BOOL_OPT(OPT_NAME, SAVE_TO)                         \
    ...
    static const char short_opts[] = "dSsRt:hD:o:v::x::Vr:jJ:lW:L:M:B:C:";
    static struct option long_opts[] = {
        ...
        BOOL_OPT("xfer-server", &opts.use_xfer_server),
        { "no-prestore", no_argument, &opts.no_prestore, false},
        { "dump-info", required_argument, 0, 1101 },
        { "iterations", required_argument, 0, 1102 },
        { "rp", required_argument, 0, 1103 },
        { "use-mmap", no_argument, &opts.use_mmap, false},
        {},
    };
...
#undef BOOL_OPT
    ...
    while (1) {
        ...
        case 'S':
            ...
            break;
        case 'B':
            SET_CHAR_OPTS(bundle, optarg);
            break;
        case 'C':
            SET_CHAR_OPTS(container_name, optarg);
            break;
        case 'D':
        ...
        case 1100:
            ...
            break;
        case 1101:
            if (add_dump_info (optarg)) {
                pr_err("Could not add dump info when initializing config: %s\n", optarg);
                return 1;
            }
            break;
        case 1102:
